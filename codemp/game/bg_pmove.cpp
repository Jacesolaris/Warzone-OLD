...
static void PM_CrashLand( void ) {
	...
	{ //saber handles its own anims
		...
		if (WeaponIsSniperCharge(pm->ps->weapon) && pm->ps->scopeType > SCOPE_BINOCULARS)
		{
...
//---------------------------------------
{
	...
	{
		if (WeaponIsSniperCharge(pm->ps->weapon))
		{
			if ((pm->cmd.buttons & BUTTON_ATTACK) &&
...
static void PM_Weapon( void )
{
	...
	*/
	...
	if (((pm->ps->torsoAnim) == TORSO_WEAPONREADY4 ||
		...
		(!WeaponIsSniperCharge(pm->ps->weapon) || pm->ps->scopeType <= SCOPE_BINOCULARS))
	{
		if (pm->ps->weapon == WP_EMPLACED_GUN)
		{
			PM_StartTorsoAnim( BOTH_GUNSIT1 );
	...
	else if (WeaponIsSniperCharge(pm->ps->weapon) && pm->ps->scopeType > SCOPE_BINOCULARS)
	{
...
//-------------------------------------------
{
	...
	if (WeaponIsSniperCharge(pmove->ps->weapon) && pmove->ps->weaponstate == WEAPON_READY)
	{
	...
	else if (WeaponIsSniperCharge(pmove->ps->weapon)) //still perform certain checks, even if the weapon is not ready
	{
	...
	if (WeaponIsSniperCharge(pmove->ps->weapon))
	{
...
void PmoveSingle (pmove_t *pmove) {
	...
	/*
	if (WeaponIsSniperCharge(pm->ps->weapon) && pm->ps->weaponstate == WEAPON_CHARGING_ALT)
