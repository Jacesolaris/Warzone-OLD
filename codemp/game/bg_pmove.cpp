...
//[NewSaberSys]
float	pm_saberDefenseScale = 0.50f;
...
/* maybe make this an npc option instad so they don't use the manual block version
...
static void PM_WalkMove( void ) {
		...
		|| (pm->ps->fd.forcePowersActive&(1 << FP_DRAIN)))
	{
		...
		if (wishspeed > pm->ps->speed * pm_saberDefenseScale)
		{
			//Defending - which means we go walking speed
			wishspeed = pm->ps->speed * pm_saberDefenseScale;
		}
	}//[/NewSaberSys]
...
static void PM_CrashLand( void ) {
	...
	{ //saber handles its own anims
		...
		if (WeaponSniperCharge(pm->ps->weapon) && pm->ps->scopeType > SCOPE_BINOCULARS)
		{
...
qboolean PM_AdjustStandAnimForSlope( void )
{
	...
	{//in a stand of some sort?
		...
		case BOTH_DESANN_STANCE:
		case BOTH_TAVION_STANCE:
		case TRIPLE3_BLUESTANCE:
		case TRIPLE3_YELLOWSTANCE:
...
static void PM_Footsteps( void ) {
	...
	if ( (PM_InSaberAnim( (pm->ps->legsAnim) ) && !BG_SpinningSaberAnim( (pm->ps->legsAnim) ))
		...
		|| (pm->ps->legsAnim) == TRIPLE3_BLUESTANCE
		|| (pm->ps->legsAnim) == TRIPLE3_YELLOWSTANCE
		|| (pm->ps->legsAnim) == TRIPLE3_REDSTANCE
		|| (pm->ps->legsAnim) == TRIPLE3_DUALSSTANCE
		|| (pm->ps->legsAnim) == TRIPLE3_STAFFSTANCE
		//[/SaberSys]
...
void PM_BeginWeaponChange( int weapon ) {
	...
	if ((pm->cmd.buttons & BUTTON_SPECIALBUTTON1) &&
	...
	if ((int)pm->ps->torsoAnim == BOTH_A7_KICK_BF_STOP)
	{
		return;
	}
	//[/NewSaberSys]
...
//---------------------------------------
{
	...
	{
		if (WeaponSniperCharge(pm->ps->weapon))
		{
			if ((pm->cmd.buttons & BUTTON_ATTACK) &&
				pm->ps->scopeType > SCOPE_BINOCULARS &&
				pm->ps->zoomLocked)
			{
				if (!WeaponIsSniperNoCharge(pm->ps->weapon))
				{
					charging = qfalse;
					altFire = qtrue;
				}
...
static void PM_Weapon( void )
{
	...
	if (!WeaponSniperCharge(pm->ps->weapon) //not using snipers
	...
	*/
	...
	if (((pm->ps->torsoAnim) == TORSO_WEAPONREADY4 ||
		...
		(!WeaponSniperCharge(pm->ps->weapon) || pm->ps->scopeType <= SCOPE_BINOCULARS))
	{
		if (pm->ps->weapon == WP_EMPLACED_GUN)
		{
			PM_StartTorsoAnim( BOTH_GUNSIT1 );
	...
	// check for fire
	//[NewSaberSys]
	if (!(pm->cmd.buttons & BUTTON_ATTACK/*BUTTON_SPECIALBUTTON1*/))// -- Stoiss wtf SLAP BUTTON SHOULD NOT DO THIS, ATTACK HERE WE COME.
	{
		if (!(pm->cmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK)))
		{
			pm->ps->weaponTime = 0;
	...
	//[NewSaberSys]
	if ((pm->cmd.buttons & BUTTON_SPECIALBUTTON1) &&
		pm->ps->weapon != WP_MELEE && pm->ps->weapon != WP_SABER)
	{
			...
			pm->ps->groundEntityNum != ENTITYNUM_NONE)
		{
			...
			if (kickmove == -1 || kickmove == LS_HILT_BASH)
			{//add any other checks for dissalowed kicks here
				anim = BOTH_A7_KICK_F;
			}
			else
			{
				anim = saberMoveData[kickmove].animToUse;
			}
	...
	else if (WeaponSniperCharge(pm->ps->weapon) && pm->ps->scopeType > SCOPE_BINOCULARS)
	{
...
//-------------------------------------------
{
	...
	if (weaponData[pm->ps->weapon].firingType == FT_BURST)
	{
		if (pm->ps->shotsRemaining & ~SHOTS_TOGGLEBIT)
		{
			...
			if (pm->ps->weaponTime <= 0 || pm->ps->shotsRemaining <= 0)
			{
				pm->cmd.buttons &= ~BUTTON_ATTACK;
				primFireDown = qfalse;
			}
		}
		else if (!(pm->ps->shotsRemaining & ~SHOTS_TOGGLEBIT) 
	...
	{
		if ( !(pmove->ps->eFlags & EF_ALT_FIRING) 
			...
			&& !pmove->cmd.rightmove)
		{
			...
			if ( !pmove->ps->scopeType && pmove->ps->pm_type != PM_DEAD )
			{
				...
				trap->Print("DEBUG_ZOOM: enter zoom mode\n");
			}
			...
			{ //check for == 1 so we can't turn binoculars off with disruptor alt fire
				...
				pmove->ps->zoomLocked = qfalse;
				weaponData[pm->ps->weapon].firingType = weaponData[pmove->ps->weapon].firingType;
				PM_AddEvent(EV_DISRUPTOR_ZOOMSOUND);
				pmove->ps->weaponTime = 1000;
				trap->Print("DEBUG_ZOOM: exit zoom mode 1\n");
			}
		}
		else if (pmove->ps->scopeType >= SCOPE_BINOCULARS
			&& (pmove->ps->eFlags & EF_ALT_FIRING)
			&& pmove->ps->zoomLockTime < pmove->cmd.serverTime
			&& ((pmove->cmd.buttons & BUTTON_ALT_ATTACK)
				|| pmove->cmd.upmove > 0 
				|| pmove->cmd.forwardmove
				|| pmove->cmd.rightmove))
		{
			...
			weaponData[pm->ps->weapon].firingType = weaponData[pmove->ps->weapon].firingType;
			PM_AddEvent(EV_DISRUPTOR_ZOOMSOUND);
			trap->Print("DEBUG_ZOOM: exit zoom mode 2\n");
		}
		...
		else if (pmove->cmd.upmove > 0 || pmove->cmd.forwardmove || pmove->cmd.rightmove)
		{
			...
			{ //check for == 1 so we can't turn binoculars off with disruptor alt fire
				...
				weaponData[pm->ps->weapon].firingType = weaponData[pmove->ps->weapon].firingType;
				PM_AddEvent(EV_DISRUPTOR_ZOOMSOUND);
				trap->Print("DEBUG_ZOOM: exit zoom mode 3\n");
			}
		}*/
		
	}
	else if (WeaponSniperCharge(pmove->ps->weapon)) //still perform certain checks, even if the weapon is not ready
	{
		if ((pmove->cmd.buttons & BUTTON_ALT_ATTACK) || pmove->cmd.upmove > 0 || pmove->cmd.forwardmove || pmove->cmd.rightmove)
		{
			if (pmove->ps->scopeType > SCOPE_BINOCULARS && pmove->ps->zoomLockTime < pmove->cmd.serverTime)
			{ //check for == 1 so we can't turn binoculars off with disruptor alt fire
				pmove->ps->scopeType = SCOPE_NONE;
				...
				weaponData[pm->ps->weapon].firingType = weaponData[pmove->ps->weapon].firingType;
				PM_AddEvent(EV_DISRUPTOR_ZOOMSOUND);
				trap->Print("DEBUG_ZOOM: exit zoom mode 4\n");
			}
		}
	}
...
#endif //__MMO__
	{
		...
		{
			pmove->ps->eFlags |= EF_ALT_FIRING;
			trap->Print("DEBUG_ZOOM: set alt fire\n");
		}
		else
		{
			pmove->ps->eFlags &= ~EF_ALT_FIRING;
			trap->Print("DEBUG_ZOOM: exit alt fire\n");
		}
	...
	if (WeaponSniperCharge(pmove->ps->weapon))
	{
...
static QINLINE void PM_CmdForSaberMoves(usercmd_t *ucmd)
{
	...
	//[SaberSys]
	/* not sure if i need to keep or let it stay
...
void PmoveSingle (pmove_t *pmove) {
	...
	/*
	if (WeaponSniperCharge(pm->ps->weapon) && pm->ps->weaponstate == WEAPON_CHARGING_ALT)
