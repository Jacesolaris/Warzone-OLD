...
static void AnimateVehicle( Vehicle_t *pVeh )
{
	...
	{
		qboolean		Turbo		= (qboolean)(fSpeedPercToMax>0.0f && level.time<pVeh->m_iTurboTime);
		qboolean		Walking		= (qboolean)(fSpeedPercToMax>0.0f && ((pVeh->m_ucmd.buttons&BUTTON_WALKING) || fSpeedPercToMax<=0.275f));
		qboolean		Running		= (qboolean)(fSpeedPercToMax>0.275f);
...
static void AnimateRiders( Vehicle_t *pVeh )
{
	...
	{
		qboolean		HasWeapon	= (qboolean)((pilotPS->weapon != WP_NONE) && (pilotPS->weapon != WP_MELEE));
		qboolean		Attacking	= (qboolean)(HasWeapon && !!(pVeh->m_ucmd.buttons&BUTTON_ATTACK));
		qboolean		Right		= (qboolean)(pVeh->m_ucmd.rightmove>0);
		qboolean		Left		= (qboolean)(pVeh->m_ucmd.rightmove<0);
		qboolean		Turbo		= (qboolean)(fSpeedPercToMax>0.0f && level.time<pVeh->m_iTurboTime);
		qboolean		Walking		= (qboolean)(fSpeedPercToMax>0.0f && ((pVeh->m_ucmd.buttons&BUTTON_WALKING) || fSpeedPercToMax<=0.275f));
		qboolean		Running		= (qboolean)(fSpeedPercToMax>0.275f);
