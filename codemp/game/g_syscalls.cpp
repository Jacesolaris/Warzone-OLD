...
qboolean trap_InPVS( const vec3_t p1, const vec3_t p2 ) {
	return (qboolean)Q_syscall( G_IN_PVS, p1, p2 );
}
qboolean trap_InPVSIgnorePortals( const vec3_t p1, const vec3_t p2 ) {
	return (qboolean)Q_syscall( G_IN_PVS_IGNORE_PORTALS, p1, p2 );
}
void trap_AdjustAreaPortalState( sharedEntity_t *ent, qboolean open ) {
	Q_syscall( G_ADJUST_AREA_PORTAL_STATE, ent, open );
}
qboolean trap_AreasConnected( int area1, int area2 ) {
	return (qboolean)Q_syscall( G_AREAS_CONNECTED, area1, area2 );
}
...
qboolean trap_EntityContact( const vec3_t mins, const vec3_t maxs, const sharedEntity_t *ent ) {
	return (qboolean)Q_syscall( G_ENTITY_CONTACT, mins, maxs, ent );
}
...
qboolean trap_GetEntityToken( char *buffer, int bufferSize ) {
	return (qboolean)Q_syscall( G_GET_ENTITY_TOKEN, buffer, bufferSize );
}
...
qboolean trap_ROFF_Play( int entID, int roffID, qboolean doTranslation ) {
	return (qboolean)Q_syscall( G_ROFF_PLAY, entID, roffID, doTranslation );
}
qboolean trap_ROFF_Purge_Ent( int entID ) {
	return (qboolean)Q_syscall( G_ROFF_PURGE_ENT, entID );
}
...
qboolean trap_ICARUS_RegisterScript( const char *name, qboolean bCalledDuringInterrogate) {
	return (qboolean)Q_syscall(G_ICARUS_REGISTERSCRIPT, name, bCalledDuringInterrogate);
}
void trap_ICARUS_Init( void ) {
	Q_syscall(G_ICARUS_INIT);
}
qboolean trap_ICARUS_ValidEnt( sharedEntity_t *ent ) {
	return (qboolean)Q_syscall(G_ICARUS_VALIDENT, ent);
}
qboolean trap_ICARUS_IsInitialized( int entID ) {
	return (qboolean)Q_syscall(G_ICARUS_ISINITIALIZED, entID);
}
qboolean trap_ICARUS_MaintainTaskManager( int entID ) {
	return (qboolean)Q_syscall(G_ICARUS_MAINTAINTASKMANAGER, entID);
}
qboolean trap_ICARUS_IsRunning( int entID ) {
	return (qboolean)Q_syscall(G_ICARUS_ISRUNNING, entID);
}
qboolean trap_ICARUS_TaskIDPending(sharedEntity_t *ent, int taskID) {
	return (qboolean)Q_syscall(G_ICARUS_TASKIDPENDING, ent, taskID);
}
...
qboolean trap_Nav_Load( const char *filename, int checksum ) {
	return (qboolean)Q_syscall(G_NAV_LOAD, filename, checksum);
}
qboolean trap_Nav_Save( const char *filename, int checksum ) {
	return (qboolean)Q_syscall(G_NAV_SAVE, filename, checksum);
}
int trap_Nav_AddRawPoint( vec3_t point, int flags, int radius ) {
	return (int)Q_syscall(G_NAV_ADDRAWPOINT, point, flags, radius);
}
...
qboolean trap_Nav_Connected( int startID, int endID ) {
	return (qboolean)Q_syscall(G_NAV_CONNECTED, startID, endID);
}
...
qboolean trap_Nav_NodeFailed( sharedEntity_t *ent, int nodeID ) {
	return (qboolean)Q_syscall(G_NAV_NODEFAILED, ent, nodeID);
}
qboolean trap_Nav_NodesAreNeighbors( int startID, int endID ) {
	return (qboolean)Q_syscall(G_NAV_NODESARENEIGHBORS, startID, endID);
}
...
qboolean trap_Nav_CheckFailedEdge( failedEdge_t *failedEdge ) {
	return (qboolean)Q_syscall(G_NAV_CHECKFAILEDEDGE, failedEdge);
}
...
qboolean trap_Nav_RouteBlocked( int startID, int testEdgeID, int endID, int rejectRank ) {
	return (qboolean)Q_syscall(G_NAV_ROUTEBLOCKED, startID, testEdgeID, endID, rejectRank);
}
...
qboolean trap_Nav_GetPathsCalculated(void) {
	return (qboolean)Q_syscall(G_NAV_GETPATHSCALCULATED);
}
...
qboolean trap_G2API_SetSkin(void *ghoul2, int modelIndex, qhandle_t customSkin, qhandle_t renderSkin) {
	return (qboolean)Q_syscall(G_G2_SETSKIN, ghoul2, modelIndex, customSkin, renderSkin);
}
...
qboolean trap_G2API_SetBoneAngles(void *ghoul2, int modelIndex, const char *boneName, const vec3_t angles, const int flags, const int up, const int right, const int forward, qhandle_t *modelList, int blendTime , int currentTime ) {
	return (qboolean)(Q_syscall(G_G2_ANGLEOVERRIDE, ghoul2, modelIndex, boneName, angles, flags, up, right, forward, modelList, blendTime, currentTime));
}
qboolean trap_G2API_SetBoneAnim(void *ghoul2, const int modelIndex, const char *boneName, const int startFrame, const int endFrame, const int flags, const float animSpeed, const int currentTime, const float setFrame , const int blendTime ) {
	return (qboolean)Q_syscall(G_G2_PLAYANIM, ghoul2, modelIndex, boneName, startFrame, endFrame, flags, PASSFLOAT(animSpeed), currentTime, PASSFLOAT(setFrame), blendTime);
}
qboolean trap_G2API_GetBoneAnim(void *ghoul2, const char *boneName, const int currentTime, float *currentFrame, int *startFrame, int *endFrame, int *flags, float *animSpeed, int *modelList, const int modelIndex) {
	return (qboolean)Q_syscall(G_G2_GETBONEANIM, ghoul2, boneName, currentTime, currentFrame, startFrame, endFrame, flags, animSpeed, modelList, modelIndex);
}
...
qboolean trap_G2API_HasGhoul2ModelOnIndex(void *ghlInfo, int modelIndex) {
	return (qboolean)Q_syscall(G_G2_HASGHOUL2MODELONINDEX, ghlInfo, modelIndex);
}
qboolean trap_G2API_RemoveGhoul2Model(void *ghlInfo, int modelIndex) {
	return (qboolean)Q_syscall(G_G2_REMOVEGHOUL2MODEL, ghlInfo, modelIndex);
}
qboolean trap_G2API_RemoveGhoul2Models(void *ghlInfo) {
	return (qboolean)Q_syscall(G_G2_REMOVEGHOUL2MODELS, ghlInfo);
}
...
qboolean trap_G2API_SetRootSurface(void *ghoul2, const int modelIndex, const char *surfaceName) {
	return (qboolean)Q_syscall(G_G2_SETROOTSURFACE, ghoul2, modelIndex, surfaceName);
}
qboolean trap_G2API_SetSurfaceOnOff(void *ghoul2, const char *surfaceName, const int flags) {
	return (qboolean)Q_syscall(G_G2_SETSURFACEONOFF, ghoul2, surfaceName, flags);
}
qboolean trap_G2API_SetNewOrigin(void *ghoul2, const int boltIndex) {
	return (qboolean)Q_syscall(G_G2_SETNEWORIGIN, ghoul2, boltIndex);
}
qboolean trap_G2API_DoesBoneExist(void *ghoul2, int modelIndex, const char *boneName) {
	return (qboolean)Q_syscall(G_G2_DOESBONEEXIST, ghoul2, modelIndex, boneName);
}
...
qboolean trap_G2API_RagPCJConstraint(void *ghoul2, const char *boneName, vec3_t min, vec3_t max) {
	return (qboolean)Q_syscall(G_G2_RAGPCJCONSTRAINT, ghoul2, boneName, min, max);
}
qboolean trap_G2API_RagPCJGradientSpeed(void *ghoul2, const char *boneName, const float speed) {
	return (qboolean)Q_syscall(G_G2_RAGPCJGRADIENTSPEED, ghoul2, boneName, PASSFLOAT(speed));
}
qboolean trap_G2API_RagEffectorGoal(void *ghoul2, const char *boneName, vec3_t pos) {
	return (qboolean)Q_syscall(G_G2_RAGEFFECTORGOAL, ghoul2, boneName, pos);
}
qboolean trap_G2API_GetRagBonePos(void *ghoul2, const char *boneName, vec3_t pos, vec3_t entAngles, vec3_t entPos, vec3_t entScale) {
	return (qboolean)Q_syscall(G_G2_GETRAGBONEPOS, ghoul2, boneName, pos, entAngles, entPos, entScale);
}
qboolean trap_G2API_RagEffectorKick(void *ghoul2, const char *boneName, vec3_t velocity) {
	return (qboolean)Q_syscall(G_G2_RAGEFFECTORKICK, ghoul2, boneName, velocity);
}
qboolean trap_G2API_RagForceSolve(void *ghoul2, qboolean force) {
	return (qboolean)Q_syscall(G_G2_RAGFORCESOLVE, ghoul2, force);
}
qboolean trap_G2API_SetBoneIKState(void *ghoul2, int time, const char *boneName, int ikState, sharedSetBoneIKStateParams_t *params) {
	return (qboolean)Q_syscall(G_G2_SETBONEIKSTATE, ghoul2, time, boneName, ikState, params);
}
qboolean trap_G2API_IKMove(void *ghoul2, int time, sharedIKMoveParams_t *params) {
	return (qboolean)Q_syscall(G_G2_IKMOVE, ghoul2, time, params);
}
qboolean trap_G2API_RemoveBone(void *ghoul2, const char *boneName, int modelIndex) {
	return (qboolean)Q_syscall(G_G2_REMOVEBONE, ghoul2, boneName, modelIndex);
}
...
qboolean trap_G2API_OverrideServer(void *serverInstance) {
	return (qboolean)Q_syscall(G_G2_OVERRIDESERVER, serverInstance);
}
...
void QDECL G_Error( int level, const char *error, ... ) {
	va_list argptr;
	char text[1024];
...
void QDECL G_Printf( const char *msg, ... ) {
	...
	if ( ret == -1 )
		trap->Print( "G_Printf: overflow of 4096 bytes buffer\n" );
	else
		trap->Print( text );
}
