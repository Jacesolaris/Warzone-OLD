...
{//FIXME: pick up on bodies of dead buddies?
	...
	{//not confused by mindtrick
		...
		if ( NPCS.NPCInfo->scriptFlags&SCF_LOOK_FOR_ENEMIES )
		{
			...
			{//found an enemy
				//NPCS.NPCInfo->behaviorState = BS_HUNT_AND_KILL;//should be automatic now
				//NPC_AngerSound();
...
void NPC_SaberDroid_PickAttack( void )
{
	...
	cgame.
	if ( saberMoveData[NPCS.NPC->client->ps.saberMove].trailLength > 0 )
	{
		NPCS.NPC->client->ps.SaberActivateTrail( saberMoveData[NPCS.NPC->client->ps.saberMove].trailLength ); // saber trail lasts for 75ms...feel free to change this if you want it longer or shorter
	}
	else
	{
		NPCS.NPC->client->ps.SaberDeactivateTrail( 0 );
	}
	*/
...
{//attack behavior
	...
	/*
	else if ( gi.inPVS( NPCS.NPC->enemy->currentOrigin, NPCS.NPC->currentOrigin ) )
	{
		NPCS.NPCInfo->enemyLastSeenTime = level.time;
		faceEnemy = qtrue;
	}
	...
	{//try to shoot if it's time
		...
		{	
			...
			{//attack!
				...
				{
					TIMER_Set( NPCS.NPC, "attackDelay", NPCS.NPC->client->ps.weaponTime+Q_irand( 0, 1000 )+(Q_irand( 0, 2 )*500) );
				}
			}
		}
	}
...
void NPC_BSSD_Default( void )
{
	...
	{//have an enemy
		...
		{//turn saber on
			...
			//NPCS.NPC->client->ps.SaberActivate();
