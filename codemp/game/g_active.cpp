...
qboolean G_ActionButtonPressed(int buttons)
{
	...
	//[NewSaberSys]
	else if (buttons & BUTTON_SPECIALBUTTON2)
	{
		return qtrue;
	}
	else if (buttons & BUTTON_SPECIALBUTTON1)
	{
		return qtrue;
	}
	//[/NewSaberSys]
...
#define	MIN_NPC_SPEED	16.0f
	...
	*/
	...
	if (ent->client->ps.duelInProgress)
	{
		...
		{
			ent->client->ps.duelInProgress = (qboolean)0;
			G_AddEvent(ent, EV_PRIVATE_DUEL, 0);
		}
		else if (duelAgainst->health < 1 || duelAgainst->client->ps.stats[STAT_HEALTH] < 1)
		{
			ent->client->ps.duelInProgress = (qboolean)0;
			duelAgainst->client->ps.duelInProgress = (qboolean)0;
...
//	WP_SaberPositionUpdate(ent, ucmd); //check the server-side saber point, do apprioriate server-side actions (effects are cs-only)
	...
	pmove.noFootsteps = (qboolean)((dmflags.integer & DF_NO_FOOTSTEPS) > 0);
	...
	pmove.nonHumanoid = (qboolean)(ent->localAnimIndex > 0);
