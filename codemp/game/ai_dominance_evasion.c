...
qboolean DOM_MoveDirClear(int forwardmove, int rightmove, qboolean reset)
{
	...
	{//not even moving
		//trap->Print( "%d skipping walk-cliff check (not moving)\n", level.time );
		return qtrue;
	}

	if (ucmd.upmove > 0 || NPC->client->ps.fd.forceJumpCharge)
	{//Going to jump
		//trap->Print( "%d skipping walk-cliff check (going to jump)\n", level.time );
		return qtrue;
	}

	if (NPC->client->ps.groundEntityNum == ENTITYNUM_NONE)
	{//in the air
		//trap->Print( "%d skipping walk-cliff check (in air)\n", level.time );
		return qtrue;
	}
	/*
	...
	*/
	...
	{//Going to bump into something very close, don't move, just turn
		...
		{//okay to bump into enemy or goal
			//trap->Print( "%d bump into enemy/goal okay\n", level.time );
			return qtrue;
		}
		else if (reset)
		{//actually want to screw with the ucmd
			//trap->Print( "%d avoiding walk into wall (entnum %d)\n", level.time, trace.entityNum );
			ucmd.forwardmove = 0;
			ucmd.rightmove = 0;
	...
	*/
	...
	{//Not going off a cliff
		//trap->Print( "%d walk off cliff okay (droptrace in solid)\n", level.time );
		return qtrue;
	}
	...
	{//Not going off a cliff
		...
		//trap->Print( "%d walk off cliff okay will hit entnum %d at dropdist of %4.2f\n", level.time, trace.entityNum, (trace.fraction*bottom_max) );
		return qtrue;
	}
	...
	if (reset)
	{//actually want to screw with the ucmd
		//trap->Print( "%d avoiding walk off cliff\n", level.time );
...
evasionType_t DOM_Jedi_SaberBlockGo(gentity_t *self, usercmd_t *cmd, vec3_t pHitloc, vec3_t phitDir, gentity_t *incoming, float dist) //dist = 0.0f
{
	...
	if (d_JediAI.integer)
	{
		trap->Print("(%d) evading attack from height %4.2f, zdiff: %4.2f, rightdot: %4.2f\n", level.time, hitloc[2] - self->r.absmin[2], zdiff, rightdot);
	}
...
static qboolean DOM_Jedi_SaberBlock(int saberNum, int bladeNum) //saberNum = 0, bladeNum = 0
{
	...
	*/
	...
	if ( d_JediAI.integer )
	{
	trap->Print( "enemy saber dist: %4.2f\n", dist );
	}
	...
	*/
	...
	{//FIXME: sometimes he reacts when you're too far away to actually hit him
		if (d_JediAI.integer)
		{
			trap->Print(S_COLOR_RED"enemy saber dist: %4.2f\n", dist);
		}
		/*
		if ( dist < 300 //close
	...
	if (d_JediAI.integer)
	{
		trap->Print(S_COLOR_GREEN"enemy saber dist: %4.2f\n", dist);
	}
	...
	{//we did block (not dodge)
		...
		if (d_JediAI.integer)
		{
			trap->Print("Keep parry choice until: %d\n", level.time + parryReCalcTime);
		}
