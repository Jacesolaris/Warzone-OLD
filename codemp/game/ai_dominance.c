...
qboolean DOM_UseForceonLocal(bot_state_t *bs, vec3_t origin, qboolean pull)
{
	...
	{//found a pressable/hackable button
		...
		//trap->Trace(&tr, bs->eye, NULL, NULL, center, bs->client, MASK_PLAYERSOLID);
...
{// Just find the largest visible distance direction...
	...
	trap->Trace( &tr, org1, NULL, NULL, org2, NPC->s.number, MASK_PLAYERSOLID );
...
void DOM_BotMove(bot_state_t *bs, vec3_t dest, qboolean wptravel, qboolean strafe)
{
	...
	//if (Distance(bs->origin, dest) > 1024) assert(0);

	//trap->Print("Bot %s move to wp %i (dist %f).\n", g_entities[bs->client].client->pers.netname, bs->wpCurrent->index, Distance(bs->origin, dest));
}
...
void DOM_BotMoveto(bot_state_t *bs, qboolean strafe)
{
	...
	*/
	...
	if (bs->wpDestination && bs->wpCurrent
		...
		&& DistanceNoHeight(bs->origin, bs->wpCurrent->origin) <= 24//16//BOT_WPTOUCH_DISTANCE
		&& !bs->wpSpecial)
	{
		//trap->Print("WP (%i) Distance %f. HIT!\n", bs->wpCurrent->index, DistanceNoHeight(bs->origin, bs->wpCurrent->origin));
		DOM_WPTouch(bs);
	}
	/*else if(bs->wpDestination && bs->wpCurrent->index != bs->wpDestination->index
	&& DistanceNoHeight(bs->origin, bs->wpCurrent->origin) >= 16//BOT_WPTOUCH_DISTANCE
	&& !bs->wpSpecial)
	{
...
{//FIXME: if land on enemy, knock him down & jump off again
	...
	{//a more complicated jump
		...
		*/
		...
		//		trap->Print("apex is %4.2f percent from p1: ", (xy-z)*0.5/xy*100.0f);
		...
		if (!time)
		{
			//trap->Print( S_COLOR_RED"ERROR: no time in jump\n" );
			return qfalse;
		}
					...
					else
					{
						...
						*/
						...
						{//Fake a force-jump
							...
							{
								//trap->Print( "(%d) pre-checked force jump\n", level.time );
...
void DOM_BotBehave_AttackBasic(bot_state_t *bs, gentity_t* target)
{
		...
		&& g_entities[bs->client].client->ps.fd.saberAnimLevel != SS_DUAL)
	{
		...
		{
			if (g_entities[bs->client].client->ps.fd.saberAnimLevel != FORCE_LEVEL_3
				&& bs->saberPower)
...
void DOM_CheckHolsterSaber(bot_state_t *bs)
{
	/*
	if( bs->cur_ps.weapon == WP_SABER
	&& (!bs->currentEnemy || (bs->currentEnemy && Distance(bs->origin, bs->currentEnemy->r.currentOrigin) > 256)) )
	{
	if (!bs->cur_ps.saberHolstered)
	{
	Cmd_ToggleSaber_f(&g_entities[bs->client]);
...
void DOM_StandardBotAI(bot_state_t *bs, float thinktime)
{
	...
	*/
	...
	/*
	// UQ1: Try force jumping to enemies and waypoints as needed...
	if(bs->botBehave == BBEHAVE_MOVETO
	&& !bs->currentEnemy
	&& bs->wpNext
	&& bs->cur_ps.fd.forcePowerLevel[FP_LEVITATION] >= FORCE_LEVEL_1
	&& Distance(bs->cur_ps.origin, bs->wpNext->origin) <= 512)
	{
	if (bs->wpNext)
	{
	if (DistanceNoHeight(bs->cur_ps.origin, bs->wpNext->origin) <= 16
	...
	&& bs->cur_ps.fd.forcePowerLevel[FP_LEVITATION] >= FORCE_LEVEL_1
	&& Distance(bs->cur_ps.origin, bs->currentEnemy->r.currentOrigin) <= 512)
	{
	if (g_entities[bs->client].client->ps.groundEntityNum != ENTITYNUM_NONE
	&& Distance(bs->cur_ps.origin, bs->currentEnemy->r.currentOrigin) >= 128
	&& DistanceNoHeight(bs->cur_ps.origin, bs->currentEnemy->r.currentOrigin) <= 128)
	{// Wait till we get a better spot to jump from!

	}
	else if (DistanceNoHeight(bs->cur_ps.origin, bs->currentEnemy->r.currentOrigin) <= 16
