...
void JEDI_Debug(char *debugText)
{
	if (NPCS.NPC->s.weapon == WP_SABER) 
	{
		trap->Print(va("JEDI DEBUG: [%s] %s\n", NPCS.NPC->client->pers.netname, debugText));
	}
}
...
qboolean Fast_ChangeWeapon( int wp )
{
...
void Boba_FireDecide( void )
{
		...
		|| NPCS.NPC->client->ps.weapon == WP_S5_PISTOL)
	{
		if ( NPCS.NPC->health < NPCS.NPC->client->pers.maxHealth*0.5f || ENEMY_IS_BREAKABLE )
		{
			...
			NPCS.NPCInfo->burstMin = 1;
			NPCS.NPCInfo->burstMean = 3;
			NPCS.NPCInfo->burstMax = 5;
	...
	else if (NPCS.NPC->client->ps.weapon == WP_BOWCASTER)
	{
	...
	else if (NPCS.NPC->client->ps.weapon == WP_REPEATER)
	{
		if ( NPCS.NPC->health < NPCS.NPC->client->pers.maxHealth*0.5f || NPC_IsJedi(NPCS.NPC) || NPCS.NPC->s.eType == ET_PLAYER || ENEMY_IS_BREAKABLE)
		{
			...
			NPCS.NPCInfo->burstMin = 8;
			NPCS.NPCInfo->burstMean = 15;
			NPCS.NPCInfo->burstMax = 28;
			NPCS.NPCInfo->burstSpacing = Q_irand( 300, 750 );//attack debounce
		}
		else
		{
...
{
	if (!NPC_IsJedi(NPCS.NPC) && NPCS.NPC->client->ps.weapon != WP_SABER) return qfalse; // UQ1: Non-Jedi do not hunt this way...
...
{
	if (!NPC_IsJedi(NPCS.NPC) && NPCS.NPC->client->ps.weapon != WP_SABER) return qfalse; // UQ1: Non-Jedi do not track this way...
...
-------------------------
*/
void Jedi_EvasionSaber( vec3_t enemy_movedir, float enemy_dist, vec3_t enemy_dir )
{
	vec3_t	dirEnemy2Me;
	...
	{//check to see if he's coming at me
		...
		{//coming at/facing me!
			int whichDefense = 0;
			if ( NPCS.NPC->client->ps.weaponTime || NPCS.NPC->client->ps.saberInFlight || NPCS.NPC->client->ps.weapon != WP_SABER/*!NPC_IsJedi(NPCS.NPC)*/ )
...
void Jedi_SetEnemyInfo( vec3_t enemy_dest, vec3_t enemy_dir, float *enemy_dist, vec3_t enemy_movedir, float *enemy_movespeed, int prediction )
{
...
static qboolean Jedi_AttackDecide( int enemy_dist )
{
	...
	if ( NPCS.NPC->s.eFlags & EF_FAKE_NPC_BOT ||
		...
		( NPCS.NPC->client->NPC_class == CLASS_PADAWAN && NPCS.NPCInfo->rank == RANK_COMMANDER )*/NPC_IsJedi(NPCS.NPC) )
				...
				if (!BG_SaberInAttack(NPCS.NPC->client->ps.saberMove) && !Jedi_SaberBusy( NPCS.NPC ))
				{
					...
					{// Back away while attacking...
							...
							&& Q_irand( 0, 20 ) < 5)
						{
							...
							&& NPCS.NPC->client->ps.fd.forcePowerLevel[FP_DRAIN] > 0
	...
	{//attacking
#if 0
		if ( enemy_dist > 32 /*&& NPCS.NPCInfo->stats.aggression >= 4*/ )
...
{//FIXME: never does a simple short, regular jump...
	...
	if (NPC_EnemyAboveMe(NPCS.NPC) && NPC_Jump( NPCS.NPC, goal->r.currentOrigin ))
	{
		return qtrue;
	}
					...
					{//close
						...
						{//be nice and stand still for him...
							...
							*/
							...
							{
								G_UcmdMoveForDir( NPCS.NPC, &NPCS.ucmd, dir, dest );
							}
							else
							{
								TIMER_Set( NPCS.NPC, "movenone", Q_irand( 500, 1000 ) );
...
static void Jedi_Combat( void )
{
	...
	if (haveEnemy)
	{
		if (NPCS.NPC->enemy->s.groundEntityNum == ENTITYNUM_NONE)
			enemyJumping = qtrue;
	}
	...
	if (enemy_dist > 256)
	{
		if (NPC_EnemyAboveMe(NPCS.NPC) && !enemyJumping && NPC_Jump( NPCS.NPC, enemy_dest ))
		{
			//JEDI_Debug("Jump.");
			return;
		}
	...
	if ( enemy_dist > 256 && !Jedi_ClearPathToSpot( enemy_dest, NPCS.NPC->enemy->s.number ) )
	{//hunt him down
		...
		{
			if ( !enemyJumping && Jedi_TryJump( NPCS.NPC->enemy ) )
		...
		else
		{
			...
			if (Jedi_Track() || Jedi_Hunt()) 
			{
				//JEDI_Debug("Backup Pathing.");
				return;
			}
		}
	}
	...
	{//not throwing saber or using force grip
		...
		//if (attacked) JEDI_Debug("ATTACKED!!!");
...
void NPC_BSJedi_FollowLeader( void )
{
	...
	if (haveEnemy)
	{
		enemy_dist = Distance(NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.currentOrigin);
	...
	{
		enemy_dist = Distance(NPCS.NPC->r.currentOrigin, NPCS.NPCInfo->goalEntity->r.currentOrigin);
	}
	...
	if ( NPCS.NPCInfo->goalEntity )
	{
		...
		if ( NPC_EnemyAboveMe(NPCS.NPC) && !enemyJumping && NPC_Jump(NPCS.NPC, NPCS.NPCInfo->goalEntity->r.currentOrigin) )
		{
			return;
		}
		...
		{//can't get straight to him
			...
			{//no line of sight
				...
				{
					if ( !enemyJumping && Jedi_TryJump( NPCS.NPCInfo->goalEntity ) )
					{//started a jump
						return;
					}
					else
...
void BountyHunter_SelectBestWeapon( void )
{
	if ( NPCS.NPC->enemy
		...
		&& Distance( NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.currentOrigin ) > 800 )
	{
		...
		&& NPCS.NPC->client->ps.weapon != WP_EE3
...
{
	if ( NPCS.NPC->enemy
		&& NPCS.NPC->client->ps.weapon != WP_Z6_BLASTER_CANON 
		&& Distance( NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.currentOrigin ) > 800 )
	{
		Boba_ChangeWeapon( WP_Z6_BLASTER_CANON );
	}
	else if (NPCS.NPC->enemy
		&& NPCS.NPC->client->ps.weapon != WP_T21
		&& Distance( NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.currentOrigin ) > 400 )
	{
		Boba_ChangeWeapon(WP_T21);
	}
	else if (NPCS.NPC->client->ps.weapon != WP_S5_PISTOL)
	{
		Boba_ChangeWeapon(WP_S5_PISTOL);
	}
}
...
void Grenader_SelectBestWeapon( void )
{
	if ( NPCS.NPC->enemy
		&& NPCS.NPC->client->ps.weapon != WP_THERMAL
		&& Distance( NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.currentOrigin ) > 256 
	...
	else if ( NPCS.NPC->client->ps.weapon != NPCS.NPC->NPC->originalWeapon)
	{
		Boba_ChangeWeapon( NPCS.NPC->NPC->originalWeapon );
	}
}
...
void Jedi_SelectBestWeapon( void )
{
	...
	/*
	if ( NPC_HasGrenades(NPCS.NPC) )
	{
		Grenader_SelectBestWeapon();
		return;
	}
	*/
}
...
void BOT_SelectBestWeapon( void )
{
	...
	/*if ( NPCS.NPC->client->ps.weapon != WP_REPEATER 
		&& Distance( NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.currentOrigin ) > 600 )
	{
...
void NPC_SelectBestWeapon( void )
{
	...
	if ( NPC_IsBountyHunter(NPCS.NPC) )
	{
		BountyHunter_SelectBestWeapon();
		return;
	}

	if ( NPC_IsCommando(NPCS.NPC) )
	{
		Commando_SelectBestWeapon();
		return;
	}

	if (NPC_IsAdvancedGunner(NPCS.NPC) )
	{
		AdvancedGunner_SelectBestWeapon();
		return;
	}
...
{// UQ1: New code to make better use of force powers...
		...
		&& Q_irand( 0, 20 ) < 2)
	{
		...
		&& NPCS.NPC->client->ps.fd.forcePowerLevel[FP_GRIP] > 0
		&& NPC_Jedi_EnemyInForceRange()
		&& Q_irand( 0, 20 ) < 2 )
	{
		...
		&& NPCS.NPC->client->ps.fd.forcePowerLevel[FP_LIGHTNING] > 0
		&& NPC_Jedi_EnemyInForceRange()
		&& Q_irand( 0, 20 ) < 2 )
	{
	...
	*/
		...
		&& Q_irand( 0, 20 ) < 2 )
	{
		...
		NPC_FaceEnemy(qtrue);
		ForceThrow( NPCS.NPC, qfalse );
		TIMER_Set( NPCS.NPC, "push", irand(15000, 30000) );
		return qtrue;
	}
	else if ( TIMER_Done( NPCS.NPC, "pull" )
		&& (NPCS.NPC->client->ps.fd.forcePowersKnown&(1<<FP_PULL)) != 0
		&& (NPCS.NPC->client->ps.fd.forcePowersActive&(1<<FP_PULL)) == 0
		&& !NPC_Jedi_EnemyInForceRange()
		&& Q_irand( 0, 20 ) < 2 )
	{
		...
		TIMER_Set( NPCS.NPC, "pull", irand(15000, 30000) );
		return qtrue;
	}
	else if (NPCS.NPC->client->ps.weapon == WP_SABER //using saber
		&& TIMER_Done( NPCS.NPC, "saberthrow" )
		&& (NPCS.NPC->client->ps.fd.forcePowersKnown&(1<<FP_SABERTHROW)) != 0
		&& !NPC_Jedi_EnemyInForceRange()
		&& Q_irand( 0, 20 ) < 2 )
	{
		NPC_FaceEnemy(qtrue);
		...
		if (NPCS.NPC->client->ps.saberEntityState == 0  //not thrown yet
			&& NPCS.NPC->client->ps.fd.forcePowerLevel[FP_SABERTHROW] > FORCE_LEVEL_1 //2nd or 3rd level lightsaber
			&& !(NPCS.NPC->client->ps.fd.forcePowersActive&(1 << FP_SABERTHROW))
			&& !(NPCS.NPC->client->ps.saberEventFlags&SEF_INWATER) )//saber not in water
		{//hold it out there
			NPCS.ucmd.buttons |= BUTTON_ALT_ATTACK;
			TIMER_Set( NPCS.NPC, "saberthrow", irand(15000, 30000) );
		}
		else if (NPCS.NPC->client->ps.saberEntityState != SES_RETURNING  //not returning yet
			&& NPCS.NPC->client->ps.fd.forcePowerLevel[FP_SABERTHROW] > FORCE_LEVEL_1 //2nd or 3rd level lightsaber
			&& !(NPCS.NPC->client->ps.fd.forcePowersActive&(1 << FP_SABERTHROW))
			&& !(NPCS.NPC->client->ps.saberEventFlags&SEF_INWATER) )//saber not in water
		{//hold it out there
...
qboolean NPC_MoveIntoOptimalAttackPosition ( void )
{
	...
	{// Get off of their head!
		//JEDI_Debug("optimal position (get off player).");
		return qtrue;
	}
	else if (NPC_MoverCrushCheck(NPC))
	{// There is a mover gonna crush us... Step back...
		//JEDI_Debug("optimal position (crusher).");
		return qtrue;
	}
...
void NPC_BSJedi_Default( void )
{
	...
	{//have an enemy
		...
		/*else if ( NPCS.NPC->client->ps.weapon == WP_THERMAL )
		...
		if (NPCS.NPC->enemy && NPC_IsAlive(NPCS.NPC->enemy))
		{
			...
			{// Jedi/Sith. Use attack/counter system...
				...
				{// Counter...
					...
					if (NPCS.NPC->enemy->client->saber[1].model[0]) {
						dualSabers = qtrue;
					}

					if (Distance(NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.currentOrigin) < 128)
					...
					for (saberNum = 0; saberNum < (dualSabers ? MAX_SABERS : 1); saberNum++) 
					{
						...
						for (bladeNum = 0; bladeNum < NPCS.NPC->enemy->client->saber[saberNum].numBlades; bladeNum++) 
						{
							...
							if (Jedi_SaberBlock(saberNum, bladeNum))
							{
								blockFound = qtrue;
								break;
							}
					...
					{// Try to heal...
						...
						{// Try to heal...
							ForceHeal( NPCS.NPC );
							TIMER_Set( NPCS.NPC, "heal", irand(5000, 10000) );
						}
						else if ( TIMER_Done( NPCS.NPC, "drain" )
							&& !Jedi_SaberBusy( NPCS.NPC )
							&& NPCS.NPC->client->ps.fd.forcePowerLevel[FP_DRAIN] > 0
						...
						{// Try to drain them...
							NPC_FaceEnemy(qtrue);
							ForceDrain( NPCS.NPC );
							TIMER_Set( NPCS.NPC, "drain", irand(5000, 10000) );
						}
						else
