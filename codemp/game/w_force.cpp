...
void WP_InitForcePowers( gentity_t *ent ) {
	...
	if ( g_forceBasedTeams.integer ) {
		if ( ent->client->sess.sessionTeam == FACTION_EMPIRE )
			warnClient = (qboolean)!(BG_LegalizedForcePowers( forcePowers, sizeof (forcePowers), g_maxForceRank.integer, HasSetSaberOnly(), FORCE_DARKSIDE, level.gametype, g_forcePowerDisable.integer ));
		else if ( ent->client->sess.sessionTeam == FACTION_REBEL )
			warnClient = (qboolean)!(BG_LegalizedForcePowers( forcePowers, sizeof (forcePowers), g_maxForceRank.integer, HasSetSaberOnly(), FORCE_LIGHTSIDE, level.gametype, g_forcePowerDisable.integer ));
		else
			warnClient = (qboolean)!(BG_LegalizedForcePowers( forcePowers, sizeof (forcePowers), g_maxForceRank.integer, HasSetSaberOnly(), 0, level.gametype, g_forcePowerDisable.integer ));
	}
	else
		warnClient = (qboolean)!(BG_LegalizedForcePowers( forcePowers, sizeof (forcePowers), g_maxForceRank.integer, HasSetSaberOnly(), 0, level.gametype, g_forcePowerDisable.integer ));
...
{//Do a check of saberActionFlags to see if we can block lightning with Saber Defense powerlevel
	...
	{
		BlockLightning = (qboolean)(defender->client->ps.saberBlocked == BLOCKED_LIGHTNING); // This sets the blocked type, basically just copied this from eezstreets code below. I have no idea how this system works, so i'm just taking his word for it.
