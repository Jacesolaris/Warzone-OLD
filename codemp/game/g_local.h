...
typedef struct {
	...
	spawnGroup_t			spawnGroups[MAX_SPAWNS_PER_GROUP+1];			// Group spawn list for each group...
} spawnGroupList_t;

typedef struct {
	char					spawnGroupFilename[64+1];					// Filename loaded from...
	spawnGroupList_t		spawnGroupLists[RARITY_MAX+1];				// Group spawn list for each group...
} spawnGroupLists_t;

extern int					spawnGroupFilesLoaded;						// Total files number we have already loaded...
extern spawnGroupLists_t	spawnGroupData[MAX_SPAWNGROUP_FILES+1];
