...
static void S_MixChannel( mixChannel_t *ch, int speed, int count, int *output ) {
...
#else
	if ( ch->volumeChan == CHAN_VOICE 
		|| ch->volumeChan == CHAN_VOICE_ATTEN 
		|| ch->volumeChan == CHAN_VOICE_GLOBAL )
	{
		volume = s_volume->value * s_volumeVoice->value * 255;
	}
	else if ( ch->volumeChan == CHAN_LOCAL
		|| ch->volumeChan == CHAN_WEAPON
		|| ch->volumeChan == CHAN_ITEM
		|| ch->volumeChan == CHAN_BODY
		|| ch->volumeChan == CHAN_MENU1
		|| ch->volumeChan == CHAN_LESS_ATTEN
		|| ch->volumeChan == CHAN_AUTO)
	{
		volume = s_volume->value * s_volumeEffects->value * 255;
	}
	else if ( ch->volumeChan == CHAN_AMBIENT )
	{
		volume = s_volume->value * s_volumeAmbient->value * 255;
	}
	...
	if (ch->volumeChan == CHAN_VOICE || ch->volumeChan == CHAN_VOICE_ATTEN || ch->volumeChan == CHAN_VOICE_GLOBAL)
		S_MixLipSync(sound, ch);
