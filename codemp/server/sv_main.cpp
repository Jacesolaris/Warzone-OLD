...
thread *SERVER_UPDATE_THREAD1;
thread *SERVER_UPDATE_THREAD2;
thread *SERVER_UPDATE_THREAD3;
thread *SERVER_UPDATE_THREAD4;
thread *SERVER_UPDATE_THREAD5;
thread *SERVER_UPDATE_THREAD6;
thread *SERVER_UPDATE_THREAD7;

int		frameMsec;
int		startTime;

void SV_Frame_REAL ( int msec ) {
...
void SV_Frame_REAL3 ( int msec )
{
	if (com_dedicated->integer) SV_BotFrame( sv.time );
	if (!com_dedicated->integer) SV_BotFrame( sv.time + sv.timeResidual );
}

void SV_Frame_REAL4 ( int msec )
{
...
void SV_Frame_REAL5 ( int msec )
{
	// check timeouts
	SV_CheckTimeouts();
}

void SV_Frame_REAL6 ( int msec )
{
	// send messages back to the clients
	SV_SendClientMessages();
}

void SV_Frame_REAL7 ( int msec )
{
	SV_CheckCvars();
...
void SV_UpdateThread1(void * aArg)
{
	SV_Frame_REAL((int)aArg);
}

void SV_UpdateThread2(void * aArg)
{
	SV_Frame_REAL2((int)aArg);
}

void SV_UpdateThread3(void * aArg)
{
	SV_Frame_REAL3((int)aArg);
}

void SV_UpdateThread4(void * aArg)
{
	SV_Frame_REAL4((int)aArg);
}

void SV_UpdateThread5(void * aArg)
{
	SV_Frame_REAL5((int)aArg);
}

void SV_UpdateThread6(void * aArg)
{
	SV_Frame_REAL6((int)aArg);
}

void SV_UpdateThread7(void * aArg)
{
	SV_Frame_REAL7((int)aArg);
}

void SV_Frame( int msec ) {
#ifdef MULTITHREAD_SV_FRAME
	...
	SERVER_UPDATE_THREAD1 = new thread (SV_UpdateThread1, (void *)msec);
	SERVER_UPDATE_THREAD2 = new thread (SV_UpdateThread2, (void *)msec);
	...
	// UQ1: Can't thread these two...
	SV_Frame_REAL3(msec);
	SV_Frame_REAL4(msec);

	// UQ1: We can thread these 2 and run them at the same time...
	SERVER_UPDATE_THREAD5 = new thread (SV_UpdateThread5, (void *)msec);
	SERVER_UPDATE_THREAD6 = new thread (SV_UpdateThread6, (void *)msec);
	SERVER_UPDATE_THREAD7 = new thread (SV_UpdateThread7, (void *)msec);
	// UQ1: And wait for finish...
	SERVER_UPDATE_THREAD5->join();
	SERVER_UPDATE_THREAD6->join();
	SERVER_UPDATE_THREAD7->join();
	SERVER_UPDATE_THREAD5->~thread();
	SERVER_UPDATE_THREAD6->~thread();
	SERVER_UPDATE_THREAD7->~thread();

	// UQ1: And wait for finish...
	SERVER_UPDATE_THREAD1->join();
	SERVER_UPDATE_THREAD2->join();
	SERVER_UPDATE_THREAD1->~thread();
	SERVER_UPDATE_THREAD2->~thread();

#else //!MULTITHREAD_SV_FRAME
	// the menu kills the server with this cvar
	if ( sv_killserver->integer ) {
		SV_Shutdown ("Server was killed.\n");
		Cvar_Set( "sv_killserver", "0" );
