...
static VkPipeline create_pipeline(const Vk_Pipeline_Def& def) {
	...
	else if (def.state_bits & GLS_ATEST_LT_128)
		specialization_data.alpha_test_func = 2;
	//else if (def.state_bits & GLS_ATEST_GE_80)
	else if (def.state_bits & GLS_ATEST_GE_128)
...
void vk_bind_geometry() {
	...
	if (backEnd.viewParms.isPortal) {
		// Eye space transform.
		// NOTE: backEnd.ori.modelViewMatrix incorporates s_flipMatrix, so it should be taken into account 
		// when computing clipping plane too.
		float* eye_xform = push_constants + 16;
		for (int i = 0; i < 12; i++) {
			eye_xform[i] = backEnd.ori.modelViewMatrix[(i%4)*4 + i/4 ];
		}
