...
void main()
{
...
#if 0
	var_Position = (u_ModelViewProjectionMatrix * vec4(attr_Position, 1.0)).xyz;//attr_Position;
	...
	//viewRay = (u_invProjectionMatrix * vec4(vec3(attr_Position.xy * (-u_ViewInfo.g/attr_Position.z), -u_ViewInfo.g), 0.0)).xyz;
	//viewRay = (u_ModelViewMatrix * vec4(vec3(attr_Position.xy / attr_Position.z, 1.0), 1.0)).xyz;
	viewRay = -normalize(vec3(u_ModelViewMatrix * vec4(attr_Position, 1.0)));
