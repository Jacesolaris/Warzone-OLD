...
void getFlames(out vec4 fragColor, in vec2 uv, out vec4 fragGlow)
{
	...
	vec3 pos = vec3(uv * vec2(4., 1.) - vec2(0., shaderTime * .03), 1.5);
	float n = pow((fadeLR * fadeTB), 4.) * 2.4 * triNoise3D(pos, 1.4);
...
#endif //USE_SPARKS
	fragGlow.a = clamp(length(fragGlow.rgb / 3.0), 0.0, 1.0);
	fragGlow.a = clamp(fragGlow.a * 0.03/*0.015*/, 0.0, 1.0);
}
