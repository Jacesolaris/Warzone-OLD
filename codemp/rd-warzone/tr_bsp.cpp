...
void BspMap_Trace( trace_t *results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, const int passEntityNum, const int contentmask )
{
...
#define TRACEMAP_SIZE 1024
	...
	VectorSet(mapMins, 128000, 128000, 128000);
	VectorSet(mapMaxs, -128000, -128000, -128000);
	...
	for (i = 0; i < w->numsurfaces; i++)
	{
		...
		if (surf->cullinfo.type & CULLINFO_SPHERE)
		{
			...
			if (surfOrigin[0] < mapMins[0])
				mapMins[0] = surfOrigin[0];
			if (surfOrigin[0] > mapMaxs[0])
				mapMaxs[0] = surfOrigin[0];

			if (surfOrigin[1] < mapMins[1])
				mapMins[1] = surfOrigin[1];
			if (surfOrigin[1] > mapMaxs[1])
				mapMaxs[1] = surfOrigin[1];

			if (surfOrigin[2] < mapMins[2])
				mapMins[2] = surfOrigin[2];
			if (surfOrigin[2] > mapMaxs[2])
				mapMaxs[2] = surfOrigin[2];
		}
		else if (surf->cullinfo.type & CULLINFO_BOX)
		{
			if (surf->cullinfo.bounds[0][0] < mapMins[0])
				mapMins[0] = surf->cullinfo.bounds[0][0];
			if (surf->cullinfo.bounds[0][0] > mapMaxs[0])
				mapMaxs[0] = surf->cullinfo.bounds[0][0];

			if (surf->cullinfo.bounds[1][0] < mapMins[0])
				mapMins[0] = surf->cullinfo.bounds[1][0];
			if (surf->cullinfo.bounds[1][0] > mapMaxs[0])
				mapMaxs[0] = surf->cullinfo.bounds[1][0];

			if (surf->cullinfo.bounds[0][1] < mapMins[1])
				mapMins[1] = surf->cullinfo.bounds[0][1];
			if (surf->cullinfo.bounds[0][1] > mapMaxs[1])
				mapMaxs[1] = surf->cullinfo.bounds[0][1];

			if (surf->cullinfo.bounds[1][1] < mapMins[1])
				mapMins[1] = surf->cullinfo.bounds[1][1];
			if (surf->cullinfo.bounds[1][1] > mapMaxs[1])
				mapMaxs[1] = surf->cullinfo.bounds[1][1];

			if (surf->cullinfo.bounds[0][2] < mapMins[2])
				mapMins[2] = surf->cullinfo.bounds[0][2];
			if (surf->cullinfo.bounds[0][2] > mapMaxs[2])
				mapMaxs[2] = surf->cullinfo.bounds[0][2];

			if (surf->cullinfo.bounds[1][2] < mapMins[2])
				mapMins[2] = surf->cullinfo.bounds[1][2];
			if (surf->cullinfo.bounds[1][2] > mapMaxs[2])
				mapMaxs[2] = surf->cullinfo.bounds[1][2];
		}
	}
	...
	vec3_t	mapSize;
	vec3_t	pixelSize;
	vec3_t	scatterOffset;
	UINT8	*red[TRACEMAP_SIZE];
	UINT8	*green[TRACEMAP_SIZE];
	UINT8	*blue[TRACEMAP_SIZE];

	for (int i = 0; i < TRACEMAP_SIZE; i++)
	{
		red[i] = (UINT8*)malloc(sizeof(UINT8)*TRACEMAP_SIZE);
		green[i] = (UINT8*)malloc(sizeof(UINT8)*TRACEMAP_SIZE);
		blue[i] = (UINT8*)malloc(sizeof(UINT8)*TRACEMAP_SIZE);
	}

	memset(mapSize, 0, sizeof(mapSize));
	memset(pixelSize, 0, sizeof(pixelSize));
	memset(scatterOffset, 0, sizeof(scatterOffset));

	mapSize[0] = mapMaxs[0] - mapMins[0];
	mapSize[1] = mapMaxs[1] - mapMins[1];
	mapSize[2] = mapMaxs[2] - mapMins[2];
	pixelSize[0] = TRACEMAP_SIZE / mapSize[0];
	pixelSize[1] = TRACEMAP_SIZE / mapSize[1];
	scatterOffset[0] = mapSize[0] / TRACEMAP_SIZE;
	scatterOffset[1] = mapSize[1] / TRACEMAP_SIZE;
...
#pragma omp parallel for schedule(dynamic)
	//for (int x = (int)mapMins[0]; x < (int)mapMaxs[0]; x += scatterOffset[0])
	for (int imageX = 0; imageX < TRACEMAP_SIZE; imageX++)
	{
		//int imageX = TRACEMAP_SIZE - (float((mapMaxs[0] - x) / mapSize[0]) * TRACEMAP_SIZE);
		//if (imageX > TRACEMAP_SIZE-1) imageX = TRACEMAP_SIZE-1;
		int x = mapMins[0] + (imageX * scatterOffset[0]);

		//for (int y = mapMins[1]; y < mapMaxs[1]; y += scatterOffset[1])
		for (int imageY = 0; imageY < TRACEMAP_SIZE; imageY++)
		{
			//int imageY = TRACEMAP_SIZE - (float((mapMaxs[1] - y) / mapSize[1]) * TRACEMAP_SIZE);
			//if (imageY > TRACEMAP_SIZE-1) imageY = TRACEMAP_SIZE-1;
			int y = mapMins[1] + (imageY * scatterOffset[1]);
			...
			for (z = mapMaxs[2]; z > mapMins[2]; z -= 48.0)
			{
				...
				BspMap_Trace( &tr, pos, NULL, NULL, down, ENTITYNUM_NONE, /*MASK_ALL*/MASK_PLAYERSOLID|CONTENTS_WATER/*|CONTENTS_OPAQUE*/ );
				...
				*/
				...
				float DIST_FROM_ROOF = mapMaxs[2] - tr.endpos[2];
				float HEIGHT_COLOR_MULT = ((1.0 - (DIST_FROM_ROOF / mapSize[2])) * 0.5) + 0.5;
	...
	data = TRACEMAP_SIZE & 255; ri->FS_Write( &data, sizeof(data), f );	// 12 : width
	data = TRACEMAP_SIZE >> 8; ri->FS_Write( &data, sizeof(data), f );	// 13 : width
	data = TRACEMAP_SIZE & 255; ri->FS_Write( &data, sizeof(data), f );	// 14 : height
	data = TRACEMAP_SIZE >> 8; ri->FS_Write( &data, sizeof(data), f );	// 15 : height
	data = 32; ri->FS_Write( &data, sizeof(data), f );	// 16 : pixel size
	data = 0; ri->FS_Write( &data, sizeof(data), f );	// 17

	for( int i = 0; i < TRACEMAP_SIZE; i++ ) {
		for( int j = 0; j < TRACEMAP_SIZE; j++ ) {
