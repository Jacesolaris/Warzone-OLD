...
extern void R_AddInstancedModelToList(mdvModel_t *model, vec3_t origin, vec3_t angles, matrix_t model_matrix, trRefEntity_t *ent);
...
#ifdef __INSTANCED_MODELS__
	if (r_instancing->integer
		&& model->numFrames <= 1
		//&& ent->e.frame == 0 
		//&& ent->e.oldframe == 0 
		&& (ent->e.reType == RT_MODEL_INSTANCED || ent->e.reType == RT_PLANT || ent->e.reType == RT_GRASS)
		&& !ent->e.customShader 
		&& !ent->e.customSkin)
