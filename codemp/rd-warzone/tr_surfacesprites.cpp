...
static void RB_DrawVerticalSurfaceSprites( shaderStage_t *stage, shaderCommands_t *input)
{
	...
	if (r_surfaceSprites->integer >= 3)
	{
		ri->Printf(PRINT_WARNING, "=========================\n");
		ri->Printf(PRINT_WARNING, "height: %f\n", stage->ss->height);
		ri->Printf(PRINT_WARNING, "width: %f\n", stage->ss->width);
		ri->Printf(PRINT_WARNING, "density: %f\n", stage->ss->density);
		ri->Printf(PRINT_WARNING, "facing: %i\n", stage->ss->facing);
		ri->Printf(PRINT_WARNING, "fadeDist: %f\n", stage->ss->fadeDist);
		ri->Printf(PRINT_WARNING, "fadeMax: %f\n", stage->ss->fadeMax);
		ri->Printf(PRINT_WARNING, "fadeScale: %f\n", stage->ss->fadeScale);
		ri->Printf(PRINT_WARNING, "fxAlphaEnd: %f\n", stage->ss->fxAlphaEnd);
		ri->Printf(PRINT_WARNING, "fxAlphaStart: %f\n", stage->ss->fxAlphaStart);
		ri->Printf(PRINT_WARNING, "fxDuration: %f\n", stage->ss->fxDuration);
		ri->Printf(PRINT_WARNING, "surfaceSpriteType: %i\n", stage->ss->surfaceSpriteType);
		ri->Printf(PRINT_WARNING, "vertSkew: %f\n", stage->ss->vertSkew);
		ri->Printf(PRINT_WARNING, "wind: %f\n", stage->ss->wind);
		ri->Printf(PRINT_WARNING, "=========================\n");
	}
...
//						light *= alpha;
					...
					//ri->Printf(PRINT_WARNING, "vert ss. skew %f %f. facing %i\n", skew[0], skew[1], stage->ss->facing);
...
static void RB_DrawOrientedSurfaceSprites( shaderStage_t *stage, shaderCommands_t *input)
{
	...
	for (curindex=0; curindex<input->numIndexes-2; curindex+=3)
	{
		...
		vec3_t tNormal;
		R_VboUnpackNormal(tNormal, input->normal[curvert+2]);

		if (tNormal[1] < minnormal)
		{
			continue;
		}
		l1 = input->vertexColors[curvert][2];
		a1 = SSVertAlpha[curvert];
		fog1[0] = *((float *)(input->svars.texcoords[0])+(curvert<<1));
		fog1[1] = *((float *)(input->svars.texcoords[0])+(curvert<<1)+1);
		...
		l2 = input->vertexColors[curvert][2];
		a2 = SSVertAlpha[curvert];
		fog2[0] = *((float *)(input->svars.texcoords[0])+(curvert<<1));
		fog2[1] = *((float *)(input->svars.texcoords[0])+(curvert<<1)+1);
		...
		l3 = input->vertexColors[curvert][2];
		a3 = SSVertAlpha[curvert];
		fog3[0] = *((float *)(input->svars.texcoords[0])+(curvert<<1));
		fog3[1] = *((float *)(input->svars.texcoords[0])+(curvert<<1)+1);
...
extern float	glMatrix[16];

void R_WorldNormalToEntity (const vec3_t worldvec, vec3_t entvec)
{
	entvec[0] = -worldvec[0] * glMatrix[0] - worldvec[1] * glMatrix[4] + worldvec[2] * glMatrix[8];
	entvec[1] = -worldvec[0] * glMatrix[1] - worldvec[1] * glMatrix[5] + worldvec[2] * glMatrix[9];
	entvec[2] = -worldvec[0] * glMatrix[2] - worldvec[1] * glMatrix[6] + worldvec[2] * glMatrix[10];
}
...
void RB_DrawSurfaceSprites( shaderStage_t *stage, shaderCommands_t *input)
{
	...
	//
#if 0
	if ( input->fogNum && input->shader->fogPass && r_drawfog->value)
	{
		SSUsingFog = qtrue;
		SQuickSprite.StartGroup(&stage->bundle[0], glbits, input->fogNum);
	}
	else
#endif
	{
...
//			R_WorldPointToEntity (backEnd.viewParms.ori.origin, ssViewOrigin);
			...
			//R_LocalNormalToWorld (backEnd.refdef.viewaxis[1], ssViewRight);
			//R_LocalNormalToWorld (backEnd.refdef.viewaxis[2], ssViewUp);
