...
bool rcMeshLoaderObj::generateFromWorldData(void)
{
	...
	for (int i = 0; i < w->numsurfaces; i++)
	{
		...
		surf->*/
	/*
	for (int i = 0; i < tr.world->numVisibleLeafs[scene]; i++)
	{
		tr.world->
	...
	{
		addVertex(bspDrawVerts[i].xyz[0], bspDrawVerts[i].xyz[1], bspDrawVerts[i].xyz[2], vcap);
	}

	for (int i = 0; i < numBSPDrawIndexes; i+=3)
	{
		addTriangle(bspDrawIndexes[i], bspDrawIndexes[i+1], bspDrawIndexes[i+2], tcap);
	}
	...
	m_filename = finalNavmeshFilename;
#endif
	return true;
}