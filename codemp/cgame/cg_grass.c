...
int			FOLIAGE_TREE_TYPE[262144];
float		FOLIAGE_TREE_SCALE[262144];

qhandle_t	FOLIAGE_GRASS_MODEL[7] = { 0, 0, 0, 0, 0, 0, 0 };
qhandle_t	FOLIAGE_TREE_MODEL[3] = { 0, 0, 0 };
...
void FOLIAGE_AddToScreen( int num, qboolean treeOnly ) {
	...
	VectorSet(re.modelScale, FOLIAGE_GRASS_SCALE[num], FOLIAGE_GRASS_SCALE[num], FOLIAGE_GRASS_SCALE[num]*0.85);
	ScaleModelAxis( &re );
	...
	if (FOLIAGE_TREE_TYPE[num] == 0)
	{// Add a smaller version inside this one to fill the massive hole in the center of the model...
		if (FOLIAGE_GRASS_SELECTION[num] == 0)
		{// Smaller grass... Only up close...
			if (!treeOnly && !skip_smallStuff)
			{
				re.origin[2] += 12.0;
				VectorSet(re.modelScale, FOLIAGE_GRASS_SCALE[num] / 2.0, FOLIAGE_GRASS_SCALE[num] / 2.0, FOLIAGE_GRASS_SCALE[num] / 2.0);
				ScaleModelAxis( &re );
				FOLIAGE_AddFoliageEntityToScene( &re );
			}
		}
		else if (FOLIAGE_GRASS_SELECTION[num] == 2)
		{// Medium grass... Only up to mid range...
			if (!treeOnly && !skip_mediumStuff)
			{
				re.hModel = FOLIAGE_GRASS_MODEL[FOLIAGE_GRASS_SELECTION[num]];
				VectorSet(re.modelScale, FOLIAGE_GRASS_SCALE[num] * 1.5, FOLIAGE_GRASS_SCALE[num] * 1.5, FOLIAGE_GRASS_SCALE[num] * 1.5);
				ScaleModelAxis( &re );
				FOLIAGE_AddFoliageEntityToScene( &re );
			}
		}
		else if (FOLIAGE_GRASS_SELECTION[num] == 3)
		{// Smaller grass... Only up close...
			if (!treeOnly && !skip_smallStuff)
			{
				re.hModel = FOLIAGE_GRASS_MODEL[FOLIAGE_GRASS_SELECTION[num]];
		...
		else if (FOLIAGE_GRASS_SELECTION[num] == 4 || FOLIAGE_GRASS_SELECTION[num] == 5 || FOLIAGE_GRASS_SELECTION[num] == 6)
		{// Flowers... Only up close...
			if (!treeOnly && !skip_smallStuff)
			{
				re.origin[2] += 12.0;
				re.hModel = FOLIAGE_GRASS_MODEL[FOLIAGE_GRASS_SELECTION[num]];
				VectorSet(re.modelScale, FOLIAGE_GRASS_SCALE[num] * 0.2, FOLIAGE_GRASS_SCALE[num] * 0.2, FOLIAGE_GRASS_SCALE[num] * 0.2);
				ScaleModelAxis( &re );
				FOLIAGE_AddFoliageEntityToScene( &re );
			}
		}
	...
	{// Add a tree type... Always draw trees, even at distance...
		re.hModel = FOLIAGE_TREE_MODEL[FOLIAGE_TREE_TYPE[num]];
...
qboolean FOLIAGE_LoadFoliagePositions( void )
{
	...
	for (i = 0; i < FOLIAGE_NUM_POSITIONS; i++)
	{
		...
		trap->FS_Read( &FOLIAGE_TREE_TYPE[i], sizeof(int), f );
...
qboolean FOLIAGE_SaveFoliagePositions( void )
{
	...
	for (i = 0; i < FOLIAGE_NUM_POSITIONS; i++)
	{
		...
		trap->FS_Write( &FOLIAGE_TREE_TYPE[i], sizeof(int), f );
...
void FOLIAGE_DrawGrass( void )
{
	...
	{// Init/register all foliage models...
		FOLIAGE_GRASS_MODEL[0] = trap->R_RegisterModel( "models/map_objects/yavin/grass_b.md3" );
		FOLIAGE_GRASS_MODEL[1] = trap->R_RegisterModel( "models/pop/foliages/alp_weedy_c.md3" );
		FOLIAGE_GRASS_MODEL[2] = trap->R_RegisterModel( "models/pop/foliages/sch_weed_a.md3" );
		FOLIAGE_GRASS_MODEL[3] = trap->R_RegisterModel( "models/pop/foliages/sch_weed_b.md3" );
		FOLIAGE_GRASS_MODEL[4] = trap->R_RegisterModel( "models/pop/foliages/pop_flower_a.md3" );
		FOLIAGE_GRASS_MODEL[5] = trap->R_RegisterModel( "models/pop/foliages/pop_flower_b.md3" );
		FOLIAGE_GRASS_MODEL[6] = trap->R_RegisterModel( "models/pop/foliages/pop_flower_c.md3" );
		FOLIAGE_TREE_MODEL[0] = trap->R_RegisterModel( "models/map_objects/yavin/tree06_b.md3" );
		FOLIAGE_TREE_MODEL[1] = trap->R_RegisterModel( "models/map_objects/yavin/tree08_b.md3" );
		FOLIAGE_TREE_MODEL[2] = trap->R_RegisterModel( "models/map_objects/yavin/tree09_b.md3" );
	}
...
//#pragma omp parallel for schedule(dynamic) // multithreaded to speed this loop up...
	for (spot = 0; spot < FOLIAGE_NUM_POSITIONS; spot++)
	{
		...
		if ( FOLIAGE_TREE_TYPE[spot] != 0 && len > 3192 )
		{
			treeOnly = qtrue;
		}
		else if ( len > 3192 ) 
		...
		if ( len > 256 && !InFOV( FOLIAGE_POSITIONS[spot], cg.refdef.vieworg, cg.refdef.viewangles, cg.refdef.fov_x + 20, cg.refdef.fov_y + 20 ))
			continue;
...
//	vmCvar_t		mapname;
	...
	for ( i = 0; i < numberNodes; i++ )					//loop through all the nodes
	{
		...
		if (FOLIAGE_FloorIsGrassAt(vec))
		{
			...
			FOLIAGE_GRASS_SELECTION[FOLIAGE_NUM_POSITIONS] = irand(0,6);
			/*
			switch(FOLIAGE_GRASS_SELECTION[FOLIAGE_NUM_POSITIONS])
			{
			case 1:
				FOLIAGE_GRASS_ANGLES[FOLIAGE_NUM_POSITIONS] = irand(0,180);
				FOLIAGE_GRASS_SCALE[FOLIAGE_NUM_POSITIONS] = (float)((float)irand(65,95) / 100.0);
				break;
			case 2:
				FOLIAGE_GRASS_ANGLES[FOLIAGE_NUM_POSITIONS] = irand(0,180);
				FOLIAGE_GRASS_SCALE[FOLIAGE_NUM_POSITIONS] = (float)((float)irand(65,95) / 100.0);
			...
			case 4:
				FOLIAGE_GRASS_ANGLES[FOLIAGE_NUM_POSITIONS] = irand(0,180);
				FOLIAGE_GRASS_SCALE[FOLIAGE_NUM_POSITIONS] = (float)((float)irand(25,40) / 100.0);
				break;
			case 5:
				FOLIAGE_GRASS_ANGLES[FOLIAGE_NUM_POSITIONS] = irand(0,180);
				FOLIAGE_GRASS_SCALE[FOLIAGE_NUM_POSITIONS] = (float)((float)irand(25,40) / 100.0);
			...
			*/
			...
			if (irand(0,10) >= 10)
			{
				FOLIAGE_TREE_TYPE[FOLIAGE_NUM_POSITIONS] = irand(0,3);
				FOLIAGE_TREE_SCALE[FOLIAGE_NUM_POSITIONS] = (float)((float)irand(45,85) / 100.0);
			}
			else
			{
				FOLIAGE_TREE_TYPE[FOLIAGE_NUM_POSITIONS] = 0;
