...
{
    if ( ps->persistant[PERS_TEAM] == FACTION_SPECTATOR )
    {
        return qfalse;
    }
	...
	if ( ps->scopeType > SCOPE_NONE/*->zoomMode*/ )
    {
        return qfalse;
    }
...
void RenderGenericWeaponView ( void )
{
    ...
    if ( !ShouldRenderWeaponViewModel (cent, ps) )
    {
        return;
    }
