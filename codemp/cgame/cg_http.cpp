...
void OnData( const happyhttp::Response* r, void* userdata, const unsigned char* data, int n )
{
	...
	for (int upto = 0; upto < n; upto++)
		m_Data[counter+upto] = (char)data[upto];
...
{
	//trap->Print( "COMPLETE (%d bytes)\n", counter );
	//trap->Print( "USERDATA:\n%s\n", userdata);
	Print_Data(m_Data);
}

size_t GetHttpPostDataCPP(char *hostname, char *page, char *poststr, char *recvdata, bool https)
{
#ifdef WIN32
	WSAData wsaData;
	...
	try
	{
		...
		/*
		// UQ1: Grrr... No HTTPS support... Can't use it for acapela voice chat... Guess i'll need to find another library...
		if (https)
		{
			happyhttp::Connection conn( hostname, 443 );

			conn.setcallbacks( OnBegin, OnData, OnComplete, 0 );
			conn.request( "POST",
				page,
				headers,
				(const unsigned char*)poststr,
				strlen(poststr) );
		...
		*/
		{
			happyhttp::Connection conn( hostname, 80 );
...
extern "C"
{
	...
	size_t GetHttpPostData(char *hostname, char *page, char *poststr, char *recvdata, qboolean https)
	{
		return GetHttpPostDataCPP(hostname, page, poststr, recvdata, https);
	}
}