...
qboolean trap_Language_IsAsian(void) {
	return (qboolean)Q_syscall( UI_LANGUAGE_ISASIAN );
}
qboolean trap_Language_UsesSpaces(void) {
	return (qboolean)Q_syscall( UI_LANGUAGE_USESSPACES );
}
...
qboolean trap_Key_IsDown( int keynum ) {
	return (qboolean)Q_syscall( UI_KEY_ISDOWN, keynum );
}
qboolean trap_Key_GetOverstrikeMode( void ) {
	return (qboolean)Q_syscall( UI_KEY_GETOVERSTRIKEMODE );
}
...
qboolean trap_LAN_UpdateVisiblePings( int source ) {
	return (qboolean)Q_syscall( UI_LAN_UPDATEVISIBLEPINGS, source );
}
...
e_status trap_CIN_StopCinematic(int handle) {
  return (e_status)Q_syscall(UI_CIN_STOPCINEMATIC, handle);
}
e_status trap_CIN_RunCinematic (int handle) {
  return (e_status)Q_syscall(UI_CIN_RUNCINEMATIC, handle);
}
...
qboolean trap_SP_GetStringTextString(const char *text, char *buffer, int bufferLength) {
	return (qboolean)Q_syscall( UI_SP_GETSTRINGTEXTSTRING, text, buffer, bufferLength );
}
...
qboolean trap_G2API_SetSkin(void *ghoul2, int modelIndex, qhandle_t customSkin, qhandle_t renderSkin) {
	return (qboolean)Q_syscall(UI_G2_SETSKIN, ghoul2, modelIndex, customSkin, renderSkin);
}
...
qboolean trap_G2API_SetBoneAngles(void *ghoul2, int modelIndex, const char *boneName, const vec3_t angles, const int flags, const int up, const int right, const int forward, qhandle_t *modelList, int blendTime, int currentTime ) {
	return (qboolean)(Q_syscall(UI_G2_ANGLEOVERRIDE, ghoul2, modelIndex, boneName, angles, flags, up, right, forward, modelList, blendTime, currentTime));
}
qboolean trap_G2API_SetBoneAnim(void *ghoul2, const int modelIndex, const char *boneName, const int startFrame, const int endFrame, const int flags, const float animSpeed, const int currentTime, const float setFrame, const int blendTime ) {
	return (qboolean)Q_syscall(UI_G2_PLAYANIM, ghoul2, modelIndex, boneName, startFrame, endFrame, flags, PASSFLOAT(animSpeed), currentTime, PASSFLOAT(setFrame), blendTime);
}
qboolean trap_G2API_GetBoneAnim(void *ghoul2, const char *boneName, const int currentTime, float *currentFrame, int *startFrame, int *endFrame, int *flags, float *animSpeed, int *modelList, const int modelIndex) {
	return (qboolean)Q_syscall(UI_G2_GETBONEANIM, ghoul2, boneName, currentTime, currentFrame, startFrame, endFrame, flags, animSpeed, modelList, modelIndex);
}
qboolean trap_G2API_GetBoneFrame(void *ghoul2, const char *boneName, const int currentTime, float *currentFrame, int *modelList, const int modelIndex) {
	return (qboolean)Q_syscall(UI_G2_GETBONEFRAME, ghoul2, boneName, currentTime, currentFrame, modelList, modelIndex);
}
...
qboolean trap_G2API_HasGhoul2ModelOnIndex(void *ghlInfo, int modelIndex) {
	return (qboolean)Q_syscall(UI_G2_HASGHOUL2MODELONINDEX, ghlInfo, modelIndex);
}
qboolean trap_G2API_RemoveGhoul2Model(void *ghlInfo, int modelIndex) {
	return (qboolean)Q_syscall(UI_G2_REMOVEGHOUL2MODEL, ghlInfo, modelIndex);
}
...
qboolean trap_G2API_SetRootSurface(void *ghoul2, const int modelIndex, const char *surfaceName) {
	return (qboolean)Q_syscall(UI_G2_SETROOTSURFACE, ghoul2, modelIndex, surfaceName);
}
qboolean trap_G2API_SetSurfaceOnOff(void *ghoul2, const char *surfaceName, const int flags) {
	return (qboolean)Q_syscall(UI_G2_SETSURFACEONOFF, ghoul2, surfaceName, flags);
}
qboolean trap_G2API_SetNewOrigin(void *ghoul2, const int boltIndex) {
	return (qboolean)Q_syscall(UI_G2_SETNEWORIGIN, ghoul2, boltIndex);
}
...
qboolean trap_G2API_SetBoneIKState(void *ghoul2, int time, const char *boneName, int ikState, sharedSetBoneIKStateParams_t *params) {
	return (qboolean)Q_syscall(UI_G2_SETBONEIKSTATE, ghoul2, time, boneName, ikState, params);
}
qboolean trap_G2API_IKMove(void *ghoul2, int time, sharedIKMoveParams_t *params) {
	return (qboolean)Q_syscall(UI_G2_IKMOVE, ghoul2, time, params);
}
...
qboolean trap_G2API_AttachG2Model(void *ghoul2From, int modelIndexFrom, void *ghoul2To, int toBoltIndex, int toModel) {
	return (qboolean)Q_syscall(UI_G2_ATTACHG2MODEL, ghoul2From, modelIndexFrom, ghoul2To, toBoltIndex, toModel);
}
...
void QDECL UI_Printf( const char *msg, ... ) {
	...
	if ( ret == -1 )
		trap->Print( "UI_Printf: overflow of 4096 bytes buffer\n" );
	else
		trap->Print( text );
}
