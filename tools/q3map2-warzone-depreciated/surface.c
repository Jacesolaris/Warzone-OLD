...
void FilterDrawsurfsIntoTree( entity_t *e, tree_t *tree ){
	...
	for ( i = e->firstDrawSurf; i < numMapDrawSurfs; i++ )
	{
		...
		case SURFACE_FORCED_META:
		case SURFACE_META:
			Sys_FPrintf( SYS_STD, "Surface %4d: [%1d] %4d verts %s\n", numSurfs, ds->planar, ds->numVerts, si->shader );
			if ( refs == 0 ) {
		...
		if ( refs > 0 ) {
			...
			//Sys_FPrintf( SYS_STD, "%d verts %d indexes\n", ds->numVerts, ds->numIndexes );
	...
	Sys_FPrintf( SYS_STD, "%9d references\n", numRefs );
	Sys_FPrintf( SYS_STD, "%9d (%d) emitted drawsurfs\n", numSurfs, numBSPDrawSurfaces );
	Sys_FPrintf( SYS_STD, "%9d stripped face surfaces\n", numStripSurfaces );
	Sys_FPrintf( SYS_STD, "%9d fanned face surfaces\n", numFanSurfaces );
	Sys_FPrintf( SYS_STD, "%9d surface models generated\n", numSurfaceModels );
	Sys_FPrintf( SYS_STD, "%9d skybox surfaces generated\n", numSkyboxSurfaces );
	for ( i = 0; i < NUM_SURFACE_TYPES; i++ )
		Sys_FPrintf( SYS_STD, "%9d %s surfaces\n", numSurfacesByType[ i ], surfaceTypes[ i ] );

	Sys_FPrintf( SYS_STD, "%9d redundant indexes supressed, saving %d Kbytes\n", numRedundantIndexes, ( numRedundantIndexes * 4 / 1024 ) );
}
