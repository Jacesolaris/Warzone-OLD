...
side_t				*brushsides;
brush_texture_t		*side_brushtextures;
...
#ifdef SIN
textureref_t		side_newrefs[10];// MAX_MAPFILE_BRUSHSIDES]; // UQ1: Broke this.. fuck it...
#endif
		...
		if (!(s->flags & SFL_BEVEL))
		{
			...
			if (s->texinfo == TEXINFO_NODE)
			{
				if (brush->contents & CONTENTS_PLAYERCLIP)
				{
					...
					else if (loadedmaptype == MAPTYPE_JA)
					{
						if (fprintf(fp, "system/clip 0 0 0 1 1") < 0) return false;
					} //end else if
					else
					{
				...
				else if (brush->contents == CONTENTS_MONSTERCLIP)
				{
					...
					{
						if (fprintf(fp, "system/blocknpc 0 0 0 1 1") < 0) return false;
					} //end else if
					else
					{
						if (fprintf(fp, "clip 0 0 0 1 1") < 0) return false;
...
int LoadMapFromBSP(struct quakefile_s *qf)
{
	...
	//JKA BSP file
	if (idheader.ident == JA_BSP_IDENT && idheader.version == JA_BSP_VERSION)
	{
		ResetMapLoading();
		JA_LoadMapFromBSP(qf);
		JA_FreeMaxBSP();
	} //end if
	//QuakeLive BSP file
	else if (idheader.ident == QL_BSP_IDENT && idheader.version == QL_BSP_VERSION)
	{
		ResetMapLoading();
