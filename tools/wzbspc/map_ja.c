...
int	JA_BrushContents(mapbrush_t *b)
{
	...
	return contents;
} //end of the function JA_BrushContents
...
// Changes Globals:		-
//===========================================================================
void JA_DPlanes2MapPlanes(void)
{
	int i;

	for (i = 0; i < q3_numplanes; i++)
	{
		dplanes2mapplanes[i] = FindFloatPlane(ja_dplanes[i].normal, ja_dplanes[i].dist);
	} //end for
} //end of the function JA_DPlanes2MapPlanes
//===========================================================================
//
// Parameter:			-
// Returns:				-
// Changes Globals:		-
//===========================================================================
void JA_BSPBrushToMapBrush(q3_dbrush_t *bspbrush, entity_t *mapent)
{
	mapbrush_t *b;
	int i, k, n;
	side_t *side, *s2;
	int planenum;
	ja_dbrushside_t *bspbrushside;
	...
	b->leafnum = dbrushleafnums[bspbrush - ja_dbrushes];

	for (n = 0; n < bspbrush->numSides; n++)
	{
		//pointer to the bsp brush side
		bspbrushside = &ja_dbrushsides[bspbrush->firstSide + n];
		...
		if (ja_dbrushsidetextured[bspbrush->firstSide + n]) side->flags |= SFL_TEXTURED|SFL_VISIBLE;
		...
		{
			side->contents = ja_dshaders[bspbrushside->shaderNum].contentFlags;
			side->surf = ja_dshaders[bspbrushside->shaderNum].surfaceFlags;
			if (strstr(ja_dshaders[bspbrushside->shaderNum].shader, "common/hint"))
			{
		...
		bspplane = &ja_dplanes[bspbrushside->planeNum];
...
//							&& fabs(mapplanes[s2->planenum].dist - mapplanes[planenum].dist) < 0.01 )
		...
		//NOTE: check for TEXINFO_NODE, otherwise crash in JA_BrushContents
...
//		side_brushtextures[nummapbrushsides] = td;
	...
	b->contents = ja_dshaders[bspbrush->shaderNum].contentFlags;
	b->contents &= ~(CONTENTS_LADDER|CONTENTS_FOG|CONTENTS_STRUCTURAL);
//	b->contents = JA_BrushContents(b);
	//
...
//	AddBrushBevels(b);
	...
	nummapbrushes++;
	mapent->numbrushes++;
} //end of the function JA_BSPBrushToMapBrush
//===========================================================================
//
// Parameter:			-
// Returns:				-
// Changes Globals:		-
//===========================================================================
void JA_ParseBSPBrushes(entity_t *mapent)
{
...
//#pragma omp parallel for num_threads(numthreads)
	for (i = 0; i < ja_dmodels[mapent->modelnum].numBrushes; i++)
	{
		num_completed++;
...
//#pragma omp critical
			{
				printLabelledProgress("ParseBSPBrushes", num_completed, ja_dmodels[mapent->modelnum].numBrushes);
			}
		}

		JA_BSPBrushToMapBrush(&ja_dbrushes[ja_dmodels[mapent->modelnum].firstBrush + i], mapent);
	} //end for

	printLabelledProgress("ParseBSPBrushes", ja_dmodels[mapent->modelnum].numBrushes, ja_dmodels[mapent->modelnum].numBrushes);

} //end of the function JA_ParseBSPBrushes
//===========================================================================
//
// Parameter:			-
// Returns:				-
// Changes Globals:		-
//===========================================================================
qboolean JA_ParseBSPEntity(int entnum)
{
	entity_t *mapent;
	char *model;
	...
	return true;
} //end of the function JA_ParseBSPEntity
//===========================================================================
//
// Parameter:			-
// Returns:				-
...
void JA_AAS_CreateCurveBrushes(void)
{
	...
	for (i = 0; i < q3_numDrawSurfaces; i++)
	{
		...
		if (!(ja_dshaders[surface->shaderNum].contentFlags & (CONTENTS_SOLID|CONTENTS_PLAYERCLIP)))
		{
			//Log_Print("skipped non-solid curve\n");
			continue;
		} //end if
		// if this curve should not be used for AAS
		if ( ja_dshaders[surface->shaderNum].contentFlags & CONTENTS_NOBOTCLIP ) {
			continue;
		}
		//
		...
		dv_p = ja_drawVerts + surface->firstVert;
		for ( j = 0 ; j < c ; j++, dv_p++ )
		{
		...
		for (j = 0; j < pc->numFacets; j++)
		{
			...
			if (!winding)
			{
				Log_Print("WARNING: JA_AAS_CreateCurveBrushes: no winding\n");
				brush->numsides = 0;
				continue;
			} //end if
...
} //end of the function JA_AAS_CreateCurveBrushes
//===========================================================================
//
// Parameter:			-
// Returns:				-
...
} //end of the function JA_ResetMapLoading

void JA_LoadMapFromBSP(struct quakefile_s *qf)
{
	int i;

	//Log_Print("-- JA_LoadMapFromBSP --\n");
	Sys_PrintHeading("--- JA_LoadMapFromBSP ---\n");

	//loaded map type
	loadedmaptype = MAPTYPE_JA;
...
} //end of the function JA_LoadMapFromBSP
