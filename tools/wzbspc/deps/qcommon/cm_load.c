...
unsigned CM_Checksum(dheader_t *header) {
	...
	checksums[0] = CM_LumpChecksum(&header->lumps[JA_LUMP_SHADERS]);
	checksums[1] = CM_LumpChecksum(&header->lumps[JA_LUMP_LEAFS]);
	checksums[2] = CM_LumpChecksum(&header->lumps[JA_LUMP_LEAFBRUSHES]);
	checksums[3] = CM_LumpChecksum(&header->lumps[JA_LUMP_LEAFSURFACES]);
	checksums[4] = CM_LumpChecksum(&header->lumps[JA_LUMP_PLANES]);
	checksums[5] = CM_LumpChecksum(&header->lumps[JA_LUMP_BRUSHSIDES]);
	checksums[6] = CM_LumpChecksum(&header->lumps[JA_LUMP_BRUSHES]);
	checksums[7] = CM_LumpChecksum(&header->lumps[JA_LUMP_MODELS]);
	checksums[8] = CM_LumpChecksum(&header->lumps[JA_LUMP_NODES]);
	checksums[9] = CM_LumpChecksum(&header->lumps[JA_LUMP_SURFACES]);
	checksums[10] = CM_LumpChecksum(&header->lumps[JA_LUMP_DRAWVERTS]);
...
void CM_LoadMap( const char *name, qboolean clientload, int *checksum ) {
	...
	*checksum = last_checksum;
	...
	if (header.version != JA_BSP_VERSION && header.version != BSP_VERSION ) {
	...
	// load into heap
	CMod_LoadShaders( &header.lumps[JA_LUMP_SHADERS] );
	CMod_LoadLeafs (&header.lumps[JA_LUMP_LEAFS]);
	CMod_LoadLeafBrushes (&header.lumps[JA_LUMP_LEAFBRUSHES]);
	CMod_LoadLeafSurfaces (&header.lumps[JA_LUMP_LEAFSURFACES]);
	CMod_LoadPlanes (&header.lumps[JA_LUMP_PLANES]);
	CMod_LoadBrushSides (&header.lumps[JA_LUMP_BRUSHSIDES]);
	CMod_LoadBrushes (&header.lumps[JA_LUMP_BRUSHES]);
	CMod_LoadSubmodels (&header.lumps[JA_LUMP_MODELS]);
	CMod_LoadNodes (&header.lumps[JA_LUMP_NODES]);
	CMod_LoadEntityString (&header.lumps[JA_LUMP_ENTITIES]);
	CMod_LoadVisibility( &header.lumps[JA_LUMP_VISIBILITY] );
	CMod_LoadPatches( &header.lumps[JA_LUMP_SURFACES], &header.lumps[JA_LUMP_DRAWVERTS] );
