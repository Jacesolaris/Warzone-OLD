...
void CheckPlaneAgainstParents (int pnum, node_t *node)
{
	...
	for (p = node->parent; p; p = p->parent)
	{
		if (p->planenum == pnum)
		{
			Error("Tried parent p->planenum (%i) == pnum (%i)", p->planenum, pnum);
		}
	} //end for
#endif
...
void BuildTree(tree_t *tree)
{
	...
	printf("Building tree. Sorry, I can's do a percentage bar here. This may take some time!\n");
