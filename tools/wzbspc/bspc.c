...
#define BSPC_MOTD		     "BSPC for JKA with Warzone Enhancements"
...
int main (int argc, char **argv)
{
	...
	Log_Print(" BSPC         - v" BSPC_VERSION " (c) 1999 Id Software Inc.\n");
	setcolor(yellow, black);
	Log_Print(" WzBSPC       - v" BSPC_VERSION " - " __DATE__ " " __TIME__ " - by UniqueOne\n");
	...
	if (argc > 1 && i == argc)
	{
		mapplanes = malloc(sizeof(plane_t) * MAX_MAPFILE_PLANES);
		mapbrushes = malloc(sizeof(mapbrush_t) * MAX_MAPFILE_BRUSHES);
		brushsides = malloc(sizeof(side_t) * MAX_MAPFILE_BRUSHSIDES);
		side_brushtextures = malloc(sizeof(brush_texture_t) * MAX_MAPFILE_BRUSHSIDES);
		mapplaneusers = malloc(sizeof(int) * MAX_MAPFILE_PLANES);
		...
		switch(comp)
		{
			case COMP_BSP2MAP:
			{
				...
				for (qf = qfiles; qf; qf = qf->next)
				{
					...
					// Realloc big structs to save mem...
					/*mapplanes = realloc(mapplanes, sizeof(plane_t) * q3_numplanes);
					mapbrushes = realloc(mapbrushes, sizeof(mapbrush_t) * q3_numbrushes);
					brushsides = realloc(brushsides, sizeof(side_t) * q3_numbrushsides);
					side_brushtextures = realloc(side_brushtextures, sizeof(brush_texture_t) * q3_numbrushsides);
					mapplaneusers = realloc(mapplaneusers, sizeof(int) * q3_numplanes);*/
			...
			case COMP_BSP2AAS:
			{
				...
				for (qf = qfiles; qf; qf = qf->next)
				{
					...
					if (loadedmaptype == MAPTYPE_JA) AAS_CalcReachAndClusters(qf);
					//
...
#if defined(WIN32) || defined(_WIN32)
			"Example 1: wzbspc -bsp2aas d:\\quake3\\baseq3\\maps\\mymap?.bsp\n"
			"Example 2: wzbspc -bsp2aas d:\\quake3\\baseq3\\pak0.pk3\\maps/q3dm*.bsp\n"
#else
			"Example 1: wzbspc -bsp2aas /quake3/baseq3/maps/mymap?.bsp\n"
			"Example 2: wzbspc -bsp2aas /quake3/baseq3/pak0.pk3/maps/q3dm*.bsp\n"
#endif
			"\n"
			"Switches:\n"
