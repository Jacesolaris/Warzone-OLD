...
void WriteTGA24( char *filename, byte *data, int width, int height, qboolean flip )
{
	...
	buffer = (byte *)safe_malloc( width * height * 3 + 18 );
	memset( buffer, 0, 18 );
	buffer[ 2 ] = 2;
	buffer[ 12 ] = width & 255;
	buffer[ 13 ] = width >> 8;
...
{
	int			i;
	char		dirname[ MAX_OS_PATH ], filename[ MAX_OS_PATH ];
	byte		*lightmap;
...
int ImportLightmapsMain( int argc, char **argv )
{
	...
	for( i = 0, lightmap = bspLightBytes; lightmap < (bspLightBytes + numBSPLightBytes); i++, lightmap += (game->lightmapSize * game->lightmapSize * 3) )
	{
		...
		if( len < 0 )
		{
			Sys_Warning( "Unable to load lightmap image %s", filename );
			continue;
		}
...
#pragma omp critical (__ALLOC__)
		...
		if( pixels == NULL )
		{
			Sys_Warning( "Unable to load lightmap image %s\n", filename );
			continue;
		}
		if( width != game->lightmapSize || height != game->lightmapSize )
			Sys_Warning( "Lightmap image %s is not the right size (%d, %d) != (%d, %d)", filename, width, height, game->lightmapSize, game->lightmapSize );
...
void FinishRawLightmap( int num )
{
	int					i, j, c, size, *sc;
	float				is;
	surfaceInfo_t		*info;
	...
	lm->lightClusters = (int *)safe_malloc( lm->numLightClusters * sizeof( *lm->lightClusters ) );
	c = 0;
	for( i = 0; i < lm->numLightSurfaces; i++ )
	{
		/* get surface info */
	...
	if( lm->bspLuxels[ 0 ] == NULL )
		lm->bspLuxels[ 0 ] = (float *)safe_malloc( size );
	...
	if( bounce )
	{
		...
		if( lm->radLuxels[ 0 ] == NULL )
			lm->radLuxels[ 0 ] = (float *)safe_malloc( size );
	...
	if( lm->superNormals == NULL )
		lm->superNormals = (float *)safe_malloc( size );
	...
	if( deluxemap )
	{
		...
		if( lm->superDeluxels == NULL )
			lm->superDeluxels = (float *)safe_malloc( size );
		...
		if( lm->bspDeluxels == NULL )
			lm->bspDeluxels = (float *)safe_malloc( size );
		memset( lm->bspDeluxels, 0, size );

		/* allocate bsp normals storage */
		size = lm->w * lm->h * BSP_NORMAL_SIZE * sizeof( float );
		if( lm->bspNormals == NULL )
			lm->bspNormals = (float *)safe_malloc( size );
		memset( lm->bspNormals, 0, size );
	}
...
qboolean AddSurfaceToRawLightmap( int num, rawLightmap_t *lm )
{
	...
	if( lm->numLightSurfaces > 1 )
	{
		...
		if( info->sampleSize != lm->sampleSize ||
			...
			VectorCompare( info->ambient, lm->ambient ) != qtrue ||
			VectorCompare( info->minlight, lm->minlight ) != qtrue ||
			VectorCompare( info->colormod, lm->colormod ) != qtrue )
			return qfalse;
	...
	for( n = 0; n < lm->numLightSurfaces; n++ )
	{
		...
		for( i = 0; i < ds2->numVerts; i++ )
		{
			...
			if( s > (float) lm->w || t > (float) lm->h )
				Sys_Warning( lm->entityNum, "Lightmap texture coords out of range: S %1.4f > %3d || T %1.4f > %3d, shader %s", s, lm->w, t, lm->h, info2->si->shader );
		}
	}
...
void SetupSurfaceLightmaps( void )
{
	...
	{
		Sys_Warning( "Insane supersampling amount (%d) detected", superSample );
		superSample = 8;
	}
	...
	surfaceClusters = (int *)safe_malloc( maxSurfaceClusters * sizeof( *surfaceClusters ) );
	memset( surfaceClusters, 0, maxSurfaceClusters * sizeof( *surfaceClusters ) );
	
	/* allocate a list for per-surface info */
	surfaceInfos = (surfaceInfo_t *)safe_malloc( numBSPDrawSurfaces * sizeof( *surfaceInfos ) );
	...
	sortSurfaces = (int *)safe_malloc( numBSPDrawSurfaces * sizeof( int ) );
	...
	for( i = 0; i < numBSPModels; i++ )
	{
		...
		for( j = 0; j < model->numBSPSurfaces; j++ )
		{
			...
			info->shadeAngle = GetSurfaceExtraShadeAngle( num );
			info->lightmapStitch = GetSurfaceExtraLightmapStitch( num );
			GetSurfaceExtraAmbient( num, info->ambient );
			GetSurfaceExtraMinLight( num, info->minlight );
			GetSurfaceExtraMinVertexLight( num, info->minvertexlight );
			GetSurfaceExtraColormod( num, info->colormod );

			/* add shader-based and global ambient/minlight/colormod (entity-based stuff already added by surface extra data) */
			for( k = 0; k < 3; k++ )
			{
				info->ambient[ k ] = info->ambient[ k ] + info->si->ambient[ k ] + ambientColor[ k ];
				info->minlight[ k ] = max( max( info->minlight[ k ], info->si->minlight[ k ] ), minLight[ k ] );
				info->minvertexlight[ k ] = max( max( info->minvertexlight[ k ], info->si->minvertexlight[ k ] ), minVertexLight[ k ] );
				info->colormod[ k ] = info->colormod[ k ] * info->si->colormod[ k ] * colorMod[ k ];
			}
			...
			/* vortex: fixed for better work with non-meta misc_models */
	...
	numLightSurfaces = 0;
	lightSurfaces = (int *)safe_malloc( numSurfsLightmapped * sizeof( int ) );
	memset( lightSurfaces, 0, numSurfsLightmapped * sizeof( int ) );
	
	/* allocate a list of raw lightmaps */
	numRawSuperLuxels = 0;
	numRawLightmaps = 0;
	rawLightmaps = (rawLightmap_t *)safe_malloc( numSurfsLightmapped * sizeof( *rawLightmaps ) );
	...
	for( i = 0; i < numBSPDrawSurfaces; i++ )
	{
		printLabelledProgress("SetupSurfaceLightmaps", i, numBSPDrawSurfaces);
		...
		lmNum = numRawLightmaps;
		lm = &rawLightmaps[ lmNum ];
		numRawLightmaps++;
		...
		if( info->si->lightmapNoStitch == qtrue )
			lm->stitch = qfalse;
		else if( stitch == qtrue || info->si->lightmapForceStitch == qtrue )
			lm->stitch = qtrue;
		else
			lm->stitch = qfalse;
		...
		VectorCopy( info->ambient, lm->ambient );
		VectorCopy( info->minlight, lm->minlight );
		VectorCopy( info->colormod, lm->colormod );
...
/*
AllocateRawLightmaps
allocates buffers for lightmaps
*/
void AllocateSurfaceLightmaps(void)
{
	...
	for (i = 0; i < numRawLightmaps; i++)
	{
		printLabelledProgress("AllocateSurfaceLightmaps", i, numRawLightmaps);
	...
	{
		vertexLuxels[ i ] = (float *)safe_malloc( numBSPDrawVerts * VERTEX_LUXEL_SIZE * sizeof( float ) ); 
		memset( vertexLuxels[ i ], 0, numBSPDrawVerts * VERTEX_LUXEL_SIZE * sizeof( float ) );
		radVertexLuxels[ i ] = (float *)safe_malloc( numBSPDrawVerts * VERTEX_LUXEL_SIZE * sizeof( float ) );
...
marks luxels for stitching (which are done in the second pass)
*/

#define MAX_STITCH_CANDIDATES	1024
#define MAX_STITCH_LUXELS       20
...
void StitchRawLightmap( int rawLightmapNum )
{
	...
	float *luxel, *luxel2, *origin, *origin2, *normal, *normal2, stitchRadius, shadeAngle, *mins, *maxs, f;
	...
	for( j = 0; j < numRawLightmaps && numCandidates < MAX_STITCH_CANDIDATES; j++ )
	{
		...
		/* calc stitch radius */
		candidate->stitchRadius = ( 0.85f * min(a->actualSampleSize, b->actualSampleSize) ) / sqrt( sqrt( (float)superSample ) );
...
void StitchRawLightmaps( void )
{
	...
	vec3_t average;
	float *luxel, *normal, *origin, scale, d, bestD;

	/* disabled? */
	if( gridOnly || noStitch )
		return;
	...
	if (stitchLuxels != NULL)
	{
		...
		{
			printLabelledProgress("StitchRawLightmap", i, numStitchLuxels);
			...
			{
				/* new formula */
				/* find out best color to average to */
				lm = &rawLightmaps[ stitch->lightmap ];
				normal = SUPER_TRINORMAL_XY( stitch->xy );
				for( bestLuxel = -1, bestD = -1, j = 0; j < stitch->numLuxels; j++ )
				{
					lm = &rawLightmaps[ stitch->stitchLightmaps[ j ] ];
					luxel = SUPER_LUXEL_XY( 0, stitch->stitchXy[ j ] );
					cluster = SUPER_CLUSTER_XY( stitch->stitchXy[ j ] );
					if( *cluster < 0 || luxel[ 4 ] <= 0.0f )
						continue;
					origin = SUPER_TRIORIGIN_XY( stitch->stitchXy[ j ] );
					d = DotProduct( normal, origin );
					if( d > bestD )
					{
						bestLuxel = j;
						bestD = d;
				...
				/* set average color */
				lm = &rawLightmaps[ stitch->stitchLightmaps[ bestLuxel ] ];
				luxel = SUPER_LUXEL_XY( 0, stitch->stitchXy[ bestLuxel ] );
				VectorCopy( luxel, average );
				...
				/* if luxel is lit, stitch neighbours together (only at half power) */
				if( stitch->cluster >= 0 && luxel[ 4 ] > 0.0f )
				{
					for (j = 0; j < stitch->numLuxels; j++)
					{
						lm = &rawLightmaps[ stitch->stitchLightmaps[ j ] ];
...
static qboolean CompareBSPLuxels( rawLightmap_t *a, int aNum, rawLightmap_t *b, int bNum )
{
	...
	if( ( a->minlight[ 0 ] || a->minlight[ 1 ] || a->minlight[ 2 ]) && ((aNum == 0 && bNum != 0) || (aNum != 0 && bNum == 0)))
		return qfalse;
...
static qboolean ApproximateLuxel( rawLightmap_t *lm, bspDrawVert_t *dv )
{
	...
	for( lightmapNum = 0; lightmapNum < MAX_LIGHTMAPS; lightmapNum++ )
	{
		...
		if( lightmapNum == 0 )
		{
			...
			{
				/* set min color */
				if( color[ i ] < lm->minlight[ i ] )
					color[ i ] = lm->minlight[ i ];
				if( vertexColor[ i ] < lm->minlight[ i ] )	/* note NOT info->minvertexlight */
					vertexColor[ i ] = lm->minlight[ i ];
			}
		}
...
static void SetupOutLightmap( rawLightmap_t *lm, outLightmap_t *olm )
{
	...
	/* allocate buffers */
	olm->lightBits = (byte *)safe_malloc( (olm->customWidth * olm->customHeight / 8) + 8 );
	memset( olm->lightBits, 0, (olm->customWidth * olm->customHeight / 8) + 8 );
	olm->bspLightBytes = (byte *)safe_malloc( olm->customWidth * olm->customHeight * 3 );
	memset( olm->bspLightBytes, 0, olm->customWidth * olm->customHeight * 3 );
	if( deluxemap )
	{
		olm->bspDirBytes = (byte *)safe_malloc( olm->customWidth * olm->customHeight * 3 );
...
static void FindOutLightmaps( rawLightmap_t *lm )
{
	...
	outLightmap_t       *olm;
	surfaceInfo_t       *info;
	float               *luxel, *deluxel, *normal;
	vec3_t color, direction;
	byte                *pixel;
	qboolean ok;
	int lightmapSearchBlockSize;
	
	lightmapSearchBlockSize = 0;
		...
		for ( y = 0; y < yMax; y++ )
		{
			for ( x = 0; x < xMax; x++ )
			{
				...
				if ( deluxemap )
				{
					...
					/* vortex: flood lightmaps - dont store null deluxels (they will be flooded by nearest ones) */
					if ( deluxel[ 0 ] || deluxel[ 1 ] || deluxel[ 2 ] )
					{
...
{
	int					i, j, k, x, y, lx, ly, sx, sy, *cluster, mappedSamples, f, fOld, start;
	int					style, size, lightmapNum, lightmapNum2;
	float				*normal, *luxel, *bspLuxel, *bspLuxel2, *radLuxel, samples, occludedSamples;
	vec3_t				sample, occludedSample, dirSample, nmSample, colorMins, colorMaxs;
	float				*deluxel, *bspDeluxel, *bspDeluxel2, *bspNormal, *bspNormal2;
	byte				*lb;
	...
	for( i = 0; i < numRawLightmaps; i++ )
	{
		printLabelledProgress("SubsampleLightmaps", i, numRawLightmaps);
		...
		for( lightmapNum = 0; lightmapNum < MAX_LIGHTMAPS; lightmapNum++ )
		{
			...
			{
				size = lm->w * lm->h * BSP_LUXEL_SIZE * sizeof( float );
				lm->bspLuxels[ lightmapNum ] = (float *)safe_malloc( size );
			...
			if( bounce )
			{
				...
				if( lm->radLuxels[ lightmapNum ] == NULL )
					lm->radLuxels[ lightmapNum ] = (float *)safe_malloc( size );
					...
					for( ly = 0; ly < superSample; ly++ )
					{
						for( lx = 0; lx < superSample; lx++ )
						{
							...
							if( deluxemap && lightmapNum == 0 )
							{
								...
								VectorAdd( nmSample, normal, nmSample );
							}
							...
							/* handle debug */
							else if( debug && ( *cluster < 0 || luxel[ 4 ] <= 0 ) )
							{
								if( *cluster == CLUSTER_UNMAPPED ) /* yellow for unmapped luxels */
									VectorSet( luxel, 255, 204, 0 );
								else if( *cluster == CLUSTER_OCCLUDED ) /* purple for occluded luxels */
									VectorSet( luxel, 255, 0, 255 );
								else if( *cluster == CLUSTER_FLOODED ) /* blue for flooded luxels */
									VectorSet( luxel, 0, 32, 255 );
								else if( luxel[ 4 ] <= 0 ) /* green for unlit luxels */
									VectorSet( luxel, 0, 255, 0 );
			...
			if( lm->wrap[ 0 ] )
			{
				for( y = 0; y < lm->h; y++ )
				{
					...
					if( deluxemap && lightmapNum == 0 )
					{
						...
						bspNormal = BSP_NORMAL( 0, y );
						bspNormal2 = BSP_NORMAL( lm->w - 1, y );
						VectorAdd( bspNormal, bspNormal2, bspNormal );
						VectorScale( bspNormal, 0.5f, bspNormal );
						VectorCopy( bspNormal, bspNormal2 );
					}
				}
			}
			if( lm->wrap[ 1 ] )
			{
				for( x = 0; x < lm->w; x++ )
				{
					...
					if( deluxemap && lightmapNum == 0 )
					{
						...
						VectorScale( bspDeluxel, 0.5f, bspDeluxel );
						VectorCopy( bspDeluxel, bspDeluxel2 );
						bspNormal = BSP_NORMAL( x, 0 );
						bspNormal2 = BSP_NORMAL( x, lm->h - 1 );
						VectorAdd( bspNormal, bspNormal2, bspNormal );
						VectorScale( bspNormal, 0.5f, bspNormal );
						VectorCopy( bspNormal, bspNormal2 );
					}
	...
	/* -----------------------------------------------------------------
	   convert modelspace BSP deluxels to tangentspace
	   ----------------------------------------------------------------- */
	...
	if( !bouncing )
	{
		if( deluxemap && deluxemode == 1 )
		{
			...
			for( i = 0; i < numRawLightmaps; i++ )
			{
				printLabelledProgress("ConvertLightmaps", i, numRawLightmaps);
				...
				for( y = 0; y < lm->h; y++ )
				{
					for( x = 0; x < lm->w; x++ )
					{
						...
						/* early out */
						if (bspDeluxel[0] == 0.0f && bspDeluxel[1] == 0.0f && bspDeluxel[2] == 0.0f)
							continue;

						/* renormalize */
						VectorSet( myNormal, bspNormal[0], bspNormal[1], bspNormal[2] );
						VectorNormalize(myNormal, myNormal);
						...
						/* clamp deluxel z to 75 degrees to prevent strong exaggerations */
						bspDeluxel[2] = bspDeluxel[2]*2 - 1;
						bspDeluxel[2] = max(bspDeluxel[2], 0.25);
						bspDeluxel[2] = (bspDeluxel[2] + 1)/2;
					}
				}
			}
	...
	if( noCollapse == qfalse && deluxemap == qfalse )
	{
		...
		for( i = 0; i < numRawLightmaps; i++ )
		{
			printLabelledProgress("CollapseLightmaps", i, numRawLightmaps);
	...
	if( sortLightmaps == NULL )
		sortLightmaps = (int *)safe_malloc( numRawLightmaps * sizeof( int ) );
...
//		}
	...
	if( numSurfsVertexApproximated || verbose )
...
#pragma omp critical (__ALLOC__)
	...
	for( i = 0; i < numOutLightmaps; i++ )
	{
		printLabelledProgress("StoreLightmaps", i, numOutLightmaps);
...
#pragma omp critical (__ALLOC__)
		...
		if( olm->lightmapNum < 0 || olm->extLightmapNum >= 0 || externalLightmaps )
		{
			...
			if (olm->bspLightBytes)
			{
				...
				/* vortex: flood empty lightmap pixels */
	...
	for( i = 0; i < numBSPDrawSurfaces; i++ )
	{
		printLabelledProgress("ProjectLightmaps", i, numBSPDrawSurfaces);
		...
		for( j = 0; j < ds->numVerts; j++ )
		{
			...
			for( lightmapNum = 0; lightmapNum < MAX_LIGHTMAPS; lightmapNum++ )
			{
				...
				else
				{
					...
					{
						for( k = 0; k < 3; k++ )
							if( color[ k ] < info->minvertexlight[ k ] )
								color[ k ] = info->minvertexlight[ k ];
					}
					...
					{
						for( k = 0; k < 3; k++ )
							color[ k ] = color[ k ] * info->colormod[ k ];
					}
				}
				...
				if( !info->si->noVertexLight )
				{
					if( gridOnly && lightmapNum == 0 )
						ColorToBytes( color, dv[ j ].color[ lightmapNum ], 1.0f, qfalse );
					else
