...
void TidyShaders(void)
{
	...
	{
		printLabelledProgress("TidyShaders", shadernum, numBSPShaders);
	...
	for (shadernum = 0; shadernum < numBSPShaders; shadernum++)
	{
		printLabelledProgress("FindMergableShaders", shadernum, numBSPShaders);
	...
	for (shadernum = 0; shadernum < numBSPShaders; shadernum++)
	{
		printLabelledProgress("MergeShaders", shadernum, numBSPShaders);
	...
	for( modelnum = 0; modelnum < numBSPModels; modelnum++ )
	{
		...
		{
			printLabelledProgress("MergeShaders", numDrawSurfacesProcessed, numBSPDrawSurfaces);
	...
	for (brushnum = 0; brushnum < numBSPBrushes; brushnum++)
	{
		...
		{
			printLabelledProgress("UpdateBrushes", numBrushSidesProcessed, numBSPDrawSurfaces);
...
void MergeDrawSurfaces(void)
{
	...
	for( modelnum = 0; modelnum < numBSPModels; modelnum++ )
	{
		...
		for( surfacenum = 0; surfacenum < model->numBSPSurfaces; surfacenum++ )
		{
			printLabelledProgress("CreateMetaSurfaces", numDrawSurfacesProcessed, numBSPDrawSurfaces);
	...
	for( modelnum = 0; modelnum < numBSPModels; modelnum++ )
	{
		...
		for( surfacenum = 0; surfacenum < model->numBSPSurfaces; surfacenum++ )
		{
			printLabelledProgress("FindMergableSurfaces", numDrawSurfacesProcessed, numBSPDrawSurfaces);
	...
	for( modelnum = 0; modelnum < numBSPModels; modelnum++ )
	{
		...
		for( surfacenum = 0; surfacenum < numDS; surfacenum++ )
		{
			...
			printLabelledProgress("MergeDrawSurfaces", numDrawSurfacesProcessed, (numBSPDrawSurfaces - numMergedSurfaces));
	...
	for( leafnum = 0; leafnum < numBSPLeafs; leafnum++ )
	{
		...
		{
			printLabelledProgress("MergeLeafSurfaces", numLeafSurfacesProcessed, numBSPLeafSurfaces);
	...
	for( modelnum = 0; modelnum < numBSPModels; modelnum++ )
	{
		...
		for( surfacenum = 0; surfacenum < model->numBSPSurfaces; surfacenum++ )
		{
			...
			for (vertnum = 0; vertnum < ds->numVerts; vertnum++)
			{
				...
				printLabelledProgress("CreateMetaVerts", numVertsProcessed, numBSPDrawVerts);
	...
	for( modelnum = 0; modelnum < numBSPModels; modelnum++ )
	{
		...
		for( surfacenum = 0; surfacenum < model->numBSPSurfaces; surfacenum++ )
		{
			...
			for (vertnum = 0; vertnum < ds->numVerts; vertnum++)
			{
				...
				printLabelledProgress("FindMergableVerts", numVertsProcessed, numBSPDrawVerts);
