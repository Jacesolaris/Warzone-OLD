...
namespace Simplify
{
	...
	void simplify_mesh(int target_count, double agressiveness=7, bool verbose=false)
	{
		...
		{
			printLabelledProgress("Simplification", iteration, 100);
			...
			if ((verbose) && (iteration%5==0)) {
				printf("iteration %d - triangles %d threshold %f\n",iteration,triangle_count-deleted_triangles, (float)threshold);
			}
...
#pragma omp critical
		...
		printLabelledProgress("Simplification", 100, 100);
...
#pragma omp critical
	...
	void UsePicoModel ( picoModel_t *model )
	{
		...
		//Sys_Printf("PRE-DECIMATE: model %s. verts %i. indexes %i.\n", model->fileName, (int)vertices.size(), (int)triangles.size());
	}
};
///////////////////////////////////////////
