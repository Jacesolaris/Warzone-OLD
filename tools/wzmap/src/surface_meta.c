...
void MakeEntityMetaTriangles( entity_t *e )
{
	...
	{
		printLabelledProgress("MakeEntityMetaTriangles", i-e->firstDrawSurf, numMapDrawSurfs-e->firstDrawSurf);
...
void CreateEdge( vec4_t plane, vec3_t a, vec3_t b, edge_t *edge )
{
	...
	edge->edge[ 2 ] = DotProduct( a, edge->edge );
	edge->length = DotProduct( b, edge->edge ) - edge->edge[ 2 ];
...
void FixMetaTJunctions( void )
{
	...
	{
		printLabelledProgress("FixMetaTJunctions", i, numMetaTriangles);
...
void GroupMetaTriangles( void )
{
	...
	{
		printLabelledProgress("GroupMetaTriangles", i, numMetaTriangles);
