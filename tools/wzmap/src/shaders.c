...
/* vortex: colormod precalculated random grid */
#define COLORMOD_RANDOM_GRIDSIZE	64		// cells in each direction
#define COLORMOD_RANDOM_GRIDSCALE	16		// units per cell
#define COLORMOD_RANDOM_GRIDLEN		1024	// 64x16
...
*/
void ColorModBuildRandomMatrix()
{
	int i;

	for (i = 0; i < COLORMOD_RANDOM_GRIDSIZE; i++)
	{
		colorModRandom[ i ][ 0 ] = Random();
		colorModRandom[ i ][ 1 ] = Random();
		colorModRandom[ i ][ 2 ] = Random();
	}

	/* vortex: to keep proper scrolling first and last grid values should be indentical */
	colorModRandom[0][0] = colorModRandom[COLORMOD_RANDOM_GRIDSIZE-1][0];
	colorModRandom[0][1] = colorModRandom[COLORMOD_RANDOM_GRIDSIZE-1][1];
	colorModRandom[0][2] = colorModRandom[COLORMOD_RANDOM_GRIDSIZE-1][2];
}

/*
ColorModRandomSample()
returns a random value based on vertex origin
*/

float ColorModRandom(vec3_t origin, float power)
{
	vec3_t xyz;
	float p, randX, randY, randZ;
	int pf, pc;

	VectorCopy(origin, xyz);
	// repeat grid
	while(xyz[ 0 ] > COLORMOD_RANDOM_GRIDLEN)
		xyz[ 0 ] = xyz[ 0 ] - COLORMOD_RANDOM_GRIDLEN;
	while(xyz[ 0 ] < 0)
		xyz[ 0 ] = xyz[ 0 ] + COLORMOD_RANDOM_GRIDLEN;
	while(xyz[ 1 ] > COLORMOD_RANDOM_GRIDLEN)
		xyz[ 1 ] = xyz[ 1 ] - COLORMOD_RANDOM_GRIDLEN;
	while(xyz[ 1 ] < 0)
		xyz[ 1 ] = xyz[ 1 ] + COLORMOD_RANDOM_GRIDLEN;
	while(xyz[ 2 ] > COLORMOD_RANDOM_GRIDLEN)
		xyz[ 2 ] = xyz[ 2 ] - COLORMOD_RANDOM_GRIDLEN;
	while(xyz[ 2 ] < 0)
		xyz[ 2 ] = xyz[ 2 ] + COLORMOD_RANDOM_GRIDLEN;

	// sample X
	p = xyz[ 0 ] / COLORMOD_RANDOM_GRIDSCALE;
	pf = floor( p );
	pc = ceil( p );
	randX = colorModRandom[ pf ][ 0 ] + ( colorModRandom[ pc ][ 0 ] - colorModRandom[ pf ][ 0 ] ) * ( p - pf );

	// sample Y
	p = xyz[ 1 ] / COLORMOD_RANDOM_GRIDSCALE;
	pf = floor( p );
	pc = ceil( p );
	randY = colorModRandom[ pf ][ 1 ] + ( colorModRandom[ pc ][ 1 ] - colorModRandom[ pf ][ 1 ] ) * ( p - pf );

	// sample Z
	p = xyz[ 2 ] / COLORMOD_RANDOM_GRIDSCALE;
	pf = floor( p );
	pc = ceil( p );
	randZ = colorModRandom[ pf ][ 2 ] + ( colorModRandom[ pc ][ 2 ] - colorModRandom[ pf ][ 2 ] ) * ( p - pf );

	// return averaged randoms
	p = randX + randY + randZ;
	while(p > 1)
		p = p - 1;
	p = pow(p, power);
...
void ColorMod( entity_t *e, mapDrawSurface_t *ds, colorMod_t *cm, brush_t *b, int startVert, int numVerts )
{
	int				 i, j, k, x;
	float			 c;
	vec4_t			 mult, add;
	vec3_t			 a, o, dsBoundsMin, bsBoundsMax;
	bspDrawVert_t	 *dv;
	colorMod_t		 *cm2;
	qboolean         dsBounds;

	/* dummy check */
	if( cm == NULL || numVerts < 1 || ds->verts == NULL )
		return;

	/* bounds for CM_ALPHA_VEGETATION */
	dsBounds = qfalse;

	/* get brush origin */
	if (b)
	{
		VectorAdd(b->mins, b->maxs, o);
		VectorScale(o, 0.5, o);
	}
	...
	for( i = startVert; i < numVerts; i++ )
	{
		...
		for( cm2 = cm; cm2 != NULL; cm2 = cm2->next )
		{
			...
			switch( cm2->type )
			{
				...
				case CM_ALPHA_RANDOM_JITTER:
					add[ 3 ] = (cm2->data[ 0 ] + ColorModRandom(dv->xyz, cm2->data[ 2 ]) * (cm2->data[ 1 ] - cm2->data[ 0 ])) * 255.0f;
					break;

				case CM_ALPHA_RANDOM_SCALE:
					mult[ 3 ] = cm2->data[ 0 ] + ColorModRandom(dv->xyz, cm2->data[ 2 ]) * (cm2->data[ 1 ] - cm2->data[ 0 ]);
					break;

				case CM_WATERALPHA:
					if (ds->shaderInfo->compileFlags & C_LIQUID)
					{
						mult[ 3 ] = 0.0f;
						add[ 3 ] = cm2->data[ 0 ] * 255.0f;
					}
					...
					/* vortex: experimental */
				case CM_WATERSPRING:
					if (b && ds->shaderInfo->compileFlags & C_LIQUID)
					{
						// find triangle this vertex corresponding to
						// and set flow vector
						for (x = 0; x < ds->numIndexes; x += 3)
						{
							if (ds->indexes[x] == i || ds->indexes[x + 1] == i || ds->indexes[x + 2] == i)
							{
								// find center of triangle
								VectorAdd(ds->verts[ ds->indexes[ x ] ].xyz, ds->verts[ ds->indexes[ x + 1 ] ].xyz, a);
								VectorAdd(a, ds->verts[ ds->indexes[ x + 2 ] ].xyz, a);
								VectorScale(a, 0.333333, a);
								VectorSubtract(a, o, a);
								VectorNormalize(a, a);
								mult[ 0 ] = 0.0f;
								mult[ 1 ] = 0.0f;
								mult[ 2 ] = 0.0f;
								add[ 0 ] = (1.0 + add[ 0 ])*127.0f;
								add[ 1 ] = (1.0 + add[ 1 ])*127.0f;
								add[ 2 ] = 0.0f;
								break;
							}
						}
				...
				case CM_WATERWARP:
					...
					{
						mult[ 2 ] = 0.0f;
						add[ 2 ] = cm2->data[ 0 ] * 255.0;
					}
					break;

				/* vortex: experimental vegetation */
				case CM_ALPHA_VEGETATION:
					mult[ 3 ] = 0.0f;
					add[ 3 ] = 0.0f;

					/* calc bounds for this model (restore original misc_model bounds from drawsurfaces) */
					if( dsBounds == qfalse )
					{
						ClearBounds( dsBoundsMin, bsBoundsMax );
						if (ds->mapEntityNum != 0)
						{
							for( x = e->firstDrawSurf; x < numMapDrawSurfs; x++ )
							{
								if( mapDrawSurfs[ x ].mapEntityNum == ds->mapEntityNum )
								{
									AddPointToBounds( mapDrawSurfs[ x ].mins, dsBoundsMin, bsBoundsMax );
									AddPointToBounds( mapDrawSurfs[ x ].maxs, dsBoundsMin, bsBoundsMax );
								}
							}
						}
						dsBounds = qtrue;
					...
					/* vortex: two 4-bit numbers packed to single byte */
					if( dv->color[ 0 ][ 3 ] != 0 )
					{
						add[ 3 ] = 0;
						/* 1) a number of entity which used for randomization, 0 to 16 */
						if (ds->mapEntityNum == 0)
							x = ds->surfaceNum;
						else
...
void WriteMapShaderFile( void )
{
	...
	if( file == NULL )
	{
		Sys_Warning( "Unable to open map shader file %s for writing", mapShaderFile );
		return;
	}
...
/*
AllocateColorMod
allocates new colormod and attaches it to shader
*/

colorMod_t *AllocateColorMod( shaderInfo_t *si, colorModType_t type )
{
	colorMod_t *cm, *cm2;
...
qboolean R_ForceGenericShader ( const char *name )
{
	if (name && ( StringContainsWord(name, "warzone/tree") || StringContainsWord(name, "warzone\\tree")))
		return qtrue;
	/*
	else if (name && (StringContainsWord(name, "warzone/foliage") || StringContainsWord(name, "warzone\\foliage")))
		return qtrue;
	else if (name && ( StringContainsWord(name, "warzone/billboard") || StringContainsWord(name, "warzone\\billboard")))
		return qtrue;
	else if (name && (StringContainsWord(name, "gfx")))
		return qfalse;
	else if (StringContainsWord(name, "glow") || StringContainsWord(name, "icon"))
		return qfalse;
...
static void ParseShaderFile( const char *filename )
{
	...
	while( 1 )
	{
		...
		if( si != NULL )
		{
			if (StringContainsWord(si->shader, "bark")
				|| StringContainsWord(si->shader, "trunk")
				|| StringContainsWord(si->shader, "giant_tree")
				|| StringContainsWord(si->shader, "vine01")
				|| StringContainsWord(si->shader, "rock01")
				|| StringContainsWord(si->shader, "rock02")
				|| StringContainsWord(si->shader, "driftwoodbark01")
				|| StringContainsWord(si->shader, "gkbbark")
				|| StringContainsWord(si->shader, "gkbtreeaspenbarkcomp"))
			{
			...
			{// If we found no shader, then use a warzone generic one for compile :)
				...
				sprintf(strippedName, si->shader);
				StripExtension( strippedName );
				...
				si->shaderText = (char *)safe_malloc( strlen( myShader ) + 1 );
				strcpy( si->shaderText, myShader );

				strcpy( shaderText, myShader );
				strcat( shaderText, "\n" );

				//if (verbose)
					Sys_Printf("Created warzone generic shader for %s.\n", si->shader);
			}
			else if ( shaderText[ 0 ] != '\0')
#endif //___SHADER_GENERATOR___
			{
				strcat( shaderText, "\n" );
		...
		while( 1 )
		{
			...
			else if( !Q_stricmp( token, "surfaceparm" ) )
			{
				...
				if( ApplySurfaceParm( token, &si->contentFlags, &si->surfaceFlags, &si->compileFlags ) == qfalse )
					Sys_Warning( "%s: Unknown surfaceparm: \"%s\"", filename, token );
			}
			...
			/* vortex: a mimic of qer_editorimage <image>  */
			else if( !Q_stricmp( token, "q3map_textureImage" ) )
			{
				GetTokenAppend( shaderText, qfalse );
				strcpy( si->editorImagePath, token );
				DefaultExtension( si->editorImagePath, ".tga" );
			...
			/* vortex: prevent OPTIMIZE merging for dpreflect / dpwater (as planar reflection gets broken on merged water surface) */
			else if( !Q_stricmp( token, "dpreflect" ) || !Q_stricmp( token, "dpwater" ) || !Q_stricmp( token, "dp_reflect" ) || !Q_stricmp( token, "dp_water" ) )
				si->noMerge = qtrue;
			...
			else if( !Q_strncasecmp( token, "q3map_", 6 ) )
			{
				...
				else if( !Q_stricmp( token, "q3map_lightmapAxis" ) )
				{
					...
					{
						Sys_Warning( "%s: Unknown value for q3map_lightmapAxis: %s\n", filename, token );
				...
				else if( !Q_stricmp( token, "q3map_lightmapSize" ) )
				{
					...
					{
						Sys_Warning( "%s: Non power-of-two lightmap size specified (%d, %d)", filename, si->lmCustomWidth, si->lmCustomHeight );
						si->lmCustomWidth = lmCustomSize;
						si->lmCustomHeight = lmCustomSize;
					}
				}
				...
				/* q3map_aoScale - ambient occlusion scale */
				else if( !Q_stricmp( token, "q3map_aoScale" ) )
				{
					GetTokenAppend( shaderText, qfalse );
					si->aoScale = atof( token );
				}

				/* q3map_aoScale - ambient occlusion gain scale */
				else if( !Q_stricmp( token, "q3map_aoGainScale" ) )
				{
					GetTokenAppend( shaderText, qfalse );
					si->aoGainScale = atof( token );
				}

				/* q3map_ambientLight r g b - adds ambient light for surface lightmap/vertexlight (same metric as for editor, not normalized) */
				else if( !Q_stricmp( token, "q3map_ambientLight" ) )
				{
					VectorClear( si->ambient );
					GetTokenAppend( shaderText, qfalse );
					si->ambient[ 0 ] = atof( token );
					GetTokenAppend( shaderText, qfalse );
					si->ambient[ 1 ] = atof( token );
					GetTokenAppend( shaderText, qfalse );
					si->ambient[ 2 ] = atof( token );
				}

				/* q3map_minLight r g b - adds min light for surface lightmap/vertexlight (same metric as for editor, not normalized) */
				else if( !Q_stricmp( token, "q3map_minLight" ) )
				{
					VectorClear( si->minlight );
					GetTokenAppend( shaderText, qfalse );
					si->minlight[ 0 ] = atof( token );
					GetTokenAppend( shaderText, qfalse );
					si->minlight[ 1 ] = atof( token );
					GetTokenAppend( shaderText, qfalse );
					si->minlight[ 2 ] = atof( token );
				}

				/* q3map_minVertexLight r g b - adds min vertex light for surface lightmap/vertexlight (same metric as for editor, not normalized) */
				else if( !Q_stricmp( token, "q3map_minVertexLight" ) )
				{
					VectorClear( si->minvertexlight );
					GetTokenAppend( shaderText, qfalse );
					si->minvertexlight[ 0 ] = atof( token );
					GetTokenAppend( shaderText, qfalse );
					si->minvertexlight[ 1 ] = atof( token );
					GetTokenAppend( shaderText, qfalse );
					si->minvertexlight[ 2 ] = atof( token );
					minVertexLightUsed = qtrue;
				}
				...
				/* q3map_engineShader <shader> */
				else if( !Q_stricmp( token, "q3map_engineShader" ) )
				{
					GetTokenAppend( shaderText, qfalse );
					if( token[ 0 ] != '\0' )
					{
				...
				else if( !Q_stricmp( token, "q3map_tcGen" ) )
				{
					...
					{
						Sys_Warning( "%s: Unknown q3map_tcGen method: %s", filename, token );
						VectorClear( si->vecs[ 0 ] );
						VectorClear( si->vecs[ 1 ] );
					}
				}

				/* vortex: q3map_alphaLayers layer1_index layer2_index swap_layers */
				else if( !Q_stricmp( token, "q3map_alphaLayers" ) )
				{
					si->fixVertexAlpha = qtrue;

					/* layer 1 */
					GetTokenAppend( shaderText, qfalse );
					si->fixVertexAlphaLayers[ 0 ] = atoi( token );

					/* layer 2 */
					GetTokenAppend( shaderText, qfalse );
					si->fixVertexAlphaLayers[ 1 ] = atoi( token );
				}

				/* ydnar: gs mods: q3map_[color|rgb|alpha][Gen|Mod] <style> <parameters> */
				/* vortex: q3map_colorMod lightmap r g b - sets colormod for entity surfaces (lightmap/vertexlight)  */
					     ...
					     !Q_stricmp( token, "q3map_alphaGen" ) || !Q_stricmp( token, "q3map_alphaMod" ) )
				{
					...
					{
						cm = AllocateColorMod( si, CM_COLOR_SET );
					...
					{
						cm = AllocateColorMod( si, CM_ALPHA_SCALE );
						GetTokenAppend( shaderText, qfalse );
						cm->data[ 0 ] = atof( token );
					}

					/* alpha random */
					else if( alpha && !Q_stricmp( token, "random" ) )
					{
						cm = AllocateColorMod( si, CM_ALPHA_RANDOM );
						GetTokenAppend( shaderText, qfalse );
						cm->data[ 0 ] = atof( token );
						GetTokenAppend( shaderText, qfalse );
						cm->data[ 1 ] = atof( token );
						if (GetTokenAppend( shaderText, qfalse ) == qfalse )
							cm->data[ 2 ] = 1.0f;
						else
							cm->data[ 2 ] = atof( token );
					}

					/* alpha random jitter */
					else if( alpha && !Q_stricmp( token, "randomjitter" ) )
					{
						cm = AllocateColorMod( si, CM_ALPHA_RANDOM_JITTER );
						GetTokenAppend( shaderText, qfalse );
						cm->data[ 0 ] = atof( token );
						GetTokenAppend( shaderText, qfalse );
						cm->data[ 1 ] = atof( token );
					...
					/* alpha random scale */
					else if( alpha && !Q_stricmp( token, "randomscale" ) )
					{
						cm = AllocateColorMod( si, CM_ALPHA_RANDOM_SCALE );
						GetTokenAppend( shaderText, qfalse );
						cm->data[ 0 ] = atof( token );
						GetTokenAppend( shaderText, qfalse );
						cm->data[ 1 ] = atof( token );
					...
					{
						cm = AllocateColorMod( si, CM_COLOR_SCALE );
						Parse1DMatrixAppend( shaderText, 3, cm->data );
					}
					...
					{
						cm = AllocateColorMod( si, (colorModType_t)(CM_COLOR_DOT_PRODUCT + alpha) );
						Parse1DMatrixAppend( shaderText, 3, cm->data );
					}
					...
					{
						cm = AllocateColorMod( si, (colorModType_t)(CM_COLOR_DOT_PRODUCT_2 + alpha) );
						Parse1DMatrixAppend( shaderText, 3, cm->data );
					}
					...
					{
						cm = AllocateColorMod( si, (colorModType_t)(CM_COLOR_INVERT + alpha) );
					}
					...
					else if( !Q_stricmp( token, "volume" ) )
					{
						...
						cm = AllocateColorMod( si, CM_VOLUME );
					}

					/* wateralpha A */
					else if( !Q_stricmp( token, "wateralpha" ) )
					{
						cm = AllocateColorMod( si, CM_WATERALPHA );
						GetTokenAppend( shaderText, qfalse );
						cm->data[ 0 ] = atof( token );
					}

					/* waterspring A */
					else if( !Q_stricmp( token, "waterspring" ) )
					{
						cm = AllocateColorMod( si, CM_WATERSPRING );
						GetTokenAppend( shaderText, qfalse );
						cm->data[ 0 ] = atof( token );
					}

					/* watermove A F */
					else if( !Q_stricmp( token, "watermove" ) )
					{
						cm = AllocateColorMod( si, CM_WATERMOVE );
						GetTokenAppend( shaderText, qfalse );
						cm->data[ 0 ] = atof( token );
						GetTokenAppend( shaderText, qfalse );
						cm->data[ 1 ] = atof( token );
					...
					/* waterwarp A */
					else if( !Q_stricmp( token, "waterwarp" ) )
					{
						cm = AllocateColorMod( si, CM_WATERWARP );
						GetTokenAppend( shaderText, qfalse );
						cm->data[ 0 ] = atof( token );
					}
					...
					else if( !Q_stricmp( token, "lightmap" ) )
					{
						VectorSet( si->colormod, 1, 1, 1 );
						GetTokenAppend( shaderText, qfalse );
						si->colormod[ 0 ] = atof( token );
						GetTokenAppend( shaderText, qfalse );
						si->colormod[ 1 ] = atof( token );
						GetTokenAppend( shaderText, qfalse );
				...
				else if( !Q_stricmp( token, "q3map_tcMod" ) )
				{
					...
					else if( !Q_stricmp( token, "rotate" ) )
					{
						...
						Sys_Warning( "%s: Unknown q3map_tcMod method: %s", filename, token );
				}
			...
			)
		{
			...
			else if (StringContainsWord(si->shader, "rock01")
				|| StringContainsWord(si->shader, "rock02")
				|| StringContainsWord(si->shader, "driftwoodbark01")
				|| StringContainsWord(si->shader, "gkbbark")
				|| StringContainsWord(si->shader, "gkbtreeaspenbarkcomp"))
...
static void ParseCustomInfoParms( void )
{
	...
	if( !parsedContent )
	{
		Sys_Warning( "custinfoparms.txt: Couldn't find valid custom contentsflag section" );
		return;
	}
	...
	/* any content? */
	if( !parsedContent )
		Sys_Warning( "custinfoparms.txt: Couldn't find valid custom surfaceflag section" );
}
...
void LoadShaderInfo( void )
{
	...
	/* build colormod random matrix (to ensure vertexes with same origin have same random value) */
	ColorModBuildRandomMatrix();

	/* emit some statistics */
	Sys_Printf( "%9d materials\n", numShaderInfo );
}
