...
static void CopyBrushSidesLump( ibspHeader_t *header )
{
	...
	/* copy */
	in = (ibspBrushSide_t *)GetLump( (bspHeader_t*) header, LUMP_BRUSHSIDES );
	out = bspBrushSides;
...
static void AddBrushSidesLump( FILE *file, ibspHeader_t *header )
{
	...
	buffer = (ibspBrushSide_t *)safe_malloc( size );
...
static void CopyDrawSurfacesLump( ibspHeader_t *header )
{
	...
	/* copy */
	in = (ibspDrawSurface_t *)GetLump( (bspHeader_t*) header, LUMP_SURFACES );
	out = bspDrawSurfaces;
...
static void AddDrawSurfacesLump( FILE *file, ibspHeader_t *header )
{
	...
	buffer = (ibspDrawSurface_t *)safe_malloc( size );
	memset( buffer, 0, size );
...
static void CopyDrawVertsLump( ibspHeader_t *header )
{
	...
	/* copy */
	in = (ibspDrawVert_t *)GetLump( (bspHeader_t*) header, LUMP_DRAWVERTS );
	out = bspDrawVerts;
...
static void AddDrawVertsLump( FILE *file, ibspHeader_t *header )
{
	...
	buffer = (ibspDrawVert_t *)safe_malloc( size );
	memset( buffer, 0, size );
...
static void CopyLightGridLumps( ibspHeader_t *header )
{
	...
	/* copy */
	in = (ibspGridPoint_t *)GetLump( (bspHeader_t*) header, LUMP_LIGHTGRID );
	out = bspGridPoints;
...
static void AddLightGridLumps( FILE *file, ibspHeader_t *header )
{
	...
	buffer = (ibspGridPoint_t *)safe_malloc( numBSPGridPoints * sizeof( *out ) );
...
{		
	...
	*((int*) (bspHeader_t*) header->ident) = *((int*) game->bspIdent);
	...
	/* add marker lump */
	time( &t );
	sprintf( marker, WZMAP_MARKER, WZMAP_VERSION, asctime( localtime( &t ) ) );
