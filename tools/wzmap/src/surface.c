...
void ClassifyEntitySurfaces( entity_t *e )
{
	...
	{
		printLabelledProgress("ClassifyEntitySurfaces", i-e->firstDrawSurf, numMapDrawSurfs-e->firstDrawSurf);
...
mapDrawSurface_t *DrawSurfaceForSide( entity_t *e, brush_t *b, side_t *s, winding_t *w, vec3_t *vertexCache, int *numVertsInCache, int maxCache )
{
	...
	for( j = 0; j < w->numpoints; j++ )
	{
		...
		{
			dv->st[ 0 ] = DotProduct( forceVecs[ 0 ], vTranslated );
			dv->st[ 1 ] = DotProduct( forceVecs[ 1 ], vTranslated );
		}
...
void SubdivideFaceSurfaces( entity_t *e, tree_t *tree )
{
	...
	{
		printLabelledProgress("SubdivideFaceSurfaces", i-e->firstDrawSurf, numBaseDrawSurfs-e->firstDrawSurf);
...
void AddEntitySurfaceModels( entity_t *e )
{
	...
	for( i = e->firstDrawSurf; i < numMapDrawSurfs; i++ )
	{
		printLabelledProgress("AddEntitySurfaceModels", i-e->firstDrawSurf, numMapDrawSurfs-e->firstDrawSurf);
...
void ApplyVertexMods(entity_t *e, qboolean showpacifier)
{
	...
	for( i = e->firstDrawSurf; i < numMapDrawSurfs; i++ )
	{
		printLabelledProgress("ApplyVertexMods", i-e->firstDrawSurf, numMapDrawSurfs-e->firstDrawSurf);
