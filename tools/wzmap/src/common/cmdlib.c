...
#pragma omp critical (__ALLOC__)
	{
		...
		if(!p)
		{
			if (memlog)
				safe_malloc_logend();
			Error ("safe_malloc failed on allocation of %.2f bytes", size);
		}
	}
	return p;
}

// VorteX: memory logging
...
#pragma omp critical (__ALLOC__)
	{
		...
		{
			safe_malloc_logend();
			sprintf(str, "%s:%i", file, line);
			for (log = 0; log < memnumlogs; log++)
				if (!strcmp(str, memlogs[log].file))
					memlogs[log].bytes += size;
			if (memnumlogs >= MEM_MAXLOGS)
				Error("MEM_MAXLOGS exceeded\n");
			// new
			strcpy(memlogs[memnumlogs].file, str);
			memlogs[memnumlogs].bytes = size;
			memnumlogs++;
		}
	...
	if (!p)
		Error("safe_malloc(%s:%i) failed on allocation of %i bytes", file, line, size);
	return p;
}
...
#pragma omp critical (__ALLOC__)
	{
		...
		{
			for (log = 0; log < memnumlogs; log++)
				fprintf(f, "%s = %i\n", memlogs[log].file, memlogs[log].bytes);
			fclose(f);
		}
	}
}
...
#pragma omp critical (__ALLOC__)
	{
		...
		{
			if (size > 1024 * 1024)
				Error ("%s: safe_malloc failed on allocation of %.2f mbytes", info, size / (1024 * 1024));
			else if (size > 1024)
				Error ("%s: safe_malloc failed on allocation of %.2f kbytes", info, size / 1024);
			else
				Error ("%s: safe_malloc failed on allocation of %.2f bytes", info, size);
		}
	}
	return p;
}
