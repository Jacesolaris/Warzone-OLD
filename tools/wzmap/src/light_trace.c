...
/*
TraceWindindAddVert() - vortex
populates trace winding with new vert using dynamical grow allocation
*/
...
void ClipTraceWinding( traceWinding_t *tw, vec4_t plane, traceWinding_t *front, traceWinding_t *back )
{
	...
	if( counts[ SIDE_BACK ] == 0 )
		TraceWindingCopy( front, tw );
	
	/* entirely on back? */
	else if( counts[ SIDE_FRONT ] == 0 )
		TraceWindingCopy( back, tw );
	...
	else
	{
		...
		for( i = 0; i < tw->numVerts; i++ )
		{
			...
			{
				case SIDE_FRONT:
					TraceWindingAddVert(front, a);
					break;
				
				case SIDE_BACK:
					TraceWindingAddVert(back, a);
					break;
			...
			TraceWindingAddVert(front, &mid);
			TraceWindingAddVert(back, &mid);
		}
	}
}
...
skipShadowGroups:
	...
	{
		/* vortex: save skylight shader we hit */
		if( si->skyLightValue )
			trace->skyLightShader = si;
		return qfalse;
	}
	...
	s = s - floor( s );
	t = t - floor( t );
	is = max(0, min(s * si->lightImage->width, si->lightImage->width - 1)); // vortex: added min here because somehow it may get out of texture bounds
	it = max(0, min(t * si->lightImage->height, si->lightImage->height - 1));
