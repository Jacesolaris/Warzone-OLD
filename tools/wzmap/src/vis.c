...
//	uncompressed[leafnum>>3] |= (1<<(leafnum&7));
	...
	//uint32_t bspLeafbytesSize = sizeof(byte) * MAX_MAP_VISIBILITY;
	//uint32_t uncompressedDataSize = sizeof(byte) * (MAX_MAP_LEAFS / 8);
	//printf("bspLeafbytesSize %u. WritePos %u.\n", bspLeafbytesSize, bspVisBytes + VIS_HEADER_SIZE + (leafnum * leafbytes));
...
//#pragma omp ordered
//		{
//			printLabelledProgress("CreateLeafVis", i, portalclusters);
//		}
	...
	*/
	...
	RunThreadsOnIndividual("CreateLeafVis", portalclusters, qtrue, ClusterMerge);

	Sys_Printf("%9d portal clusters\n", portalclusters);
...
/*
   ...
   ============
 */
uint32_t TryMergeLeaves(uint32_t l1num, uint32_t l2num){
...
/*
   ...
   ============
 */
uint32_t CountActivePortals(void){
...
void LoadPortals( char *name ){
	...
	memset((vportal_t *)portals, 0, 2 * numportals * sizeof(vportal_t));
	...
	((uint32_t *)bspVisBytes)[0] = portalclusters;
	((uint32_t *)bspVisBytes)[1] = leafbytes;
...
int VisMain( int argc, char **argv ){
	...
	Sys_Printf("Visdata Size: %i bytes (%i MB)\n", numBSPVisBytes, numBSPVisBytes / (1024 * 1024));
